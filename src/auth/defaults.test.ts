...
describe("OAuth client id defaults", () => {
  it("resolves to the shipped defaults when no oauth block is configured", () => {
    createdRoot = makeProject({});
    const cfg = loadConfig(true, createdRoot);
  ...
  it("honours an explicit override from saivage.json", () => {
    createdRoot = makeProject({
  ...
  it("interpolates env-var references in the override", () => {
