...
export class CoderAgent extends WorkerAgent {
  static async create(
    ...
    const initialMessage = await buildCoderMessage(ctx, input);
    ...
    return new CoderAgent(ctx, input, initialMessage, eagerSkillBlock, config);
  }
  ...
  constructor(
    ...
    super(ctx, input, {
      role: "coder",
      systemPrompt: loadRolePrompt("coder"),
      eagerSkillBlock,
      initialMessage,
...
async function buildCoderMessage(ctx: AgentContext, input: WorkerInput): Promise<string> {
