...
Scope: functional analysis only. No design, no implementation, no edits outside this folder. Honors the workspace rule "Architecture-first, no backward compatibility" and Saivage v2 constraints: Node 20+, ESM-only, TypeScript, providers wired via [@anthropic-ai/sdk](saivage/package.json#L28-L29), [openai](saivage/package.json#L38-L39), and [@mariozechner/pi-ai](saivage/package.json#L31-L32).
...
- Inspector and reviewer agent roles inside the runtime (Saivage product terms naming agent personas) that want to recall prior critiques on the same artifact.

Query archetypes:

- "Find skills about <topic>" — semantic match over SKILL.md descriptions and bodies.
- "Find memories where the agent previously hit <symptom>" — semantic match on memory bodies, filtered by scope (`project`, `stage`, `session`).
- "Find memories about <subsystem> authored by role=reviewer in the last 30 days" — semantic with metadata filter.
...
Producers: the target project's hand-written `docs/`, `README.md`, `AGENTS.md`, `SPEC/**`, `inspections/`. Realistic example directories: [saivage/docs/](saivage/docs/), [getrich/docs/](getrich/docs/), [getrich-v2/docs/](getrich-v2/docs/), [diedrico/docs/](diedrico/docs/), [diedrico/specs/](diedrico/specs/).

Generated TypeDoc output must be excluded. Verified split for [saivage/docs/](saivage/docs/): 48 hand-written markdown files outside `docs/api/`, and 232 generated files under [saivage/docs/api/](saivage/docs/api/). The ingest layer must exclude `docs/api/**` (and equivalent generated trees in other projects) by default; the operator can override per dataset.
...
Producers: the target project's source tree (`src/**/*.ts`, `**/*.py`, `frontend/src/**/*.vue`, etc.).

Future consumers: coder, reviewer, inspector agent roles wanting "show me functions related to X" or "find places that already call this API."
...
- Every result chunk must round-trip the original source path and line span. Downstream agents open files; metadata schema must guarantee this (§5.4).
...
- **Doc lookup by intent** ("what does the architecture say about caching"): semantic match against dataset B, filtered by `path glob` if the agent already narrowed the area (e.g. `docs/architecture/**`). Top-K mid (10–20). Recall@10 dominates.
...
- **Previous critique lookup** ("what did the reviewer agent say about this artifact last time"): semantic match against dataset A's memories, filtered by `role = 'reviewer'` and a recency window. Top-K small (3–5).
...
Anthropic does NOT offer a public embeddings API as of 2026-05. Their published documentation directs callers to Voyage AI for embeddings paired with Claude for generation. The `@anthropic-ai/sdk` exposes Messages and Files APIs but no `embeddings.create`.
...
These are interesting because the same model can later run locally (§3), enabling hosted-to-local cutover. That cutover preserves the embedding space only if the hosted runtime and the local runtime produce byte-identical vectors — same model revision, same tokenizer, same pooling, same normalization, same prompt prefix. Same model name alone does NOT guarantee this. The design must record the full provider+model+revision tuple in store metadata and refuse to mix on mismatch (see §5.6).
...
Inspection command for the implementer: `ls saivage/node_modules/@mariozechner/pi-ai/dist` and grep for `embed` in the exported surface; cross-check against the upstream repo's README.

### 2.9 Cheapest-reasonable default at Saivage scale

Ranking the candidates against the constraints "cheap, low-friction, already in Saivage's dependency tree, asymmetric-query support not required, predictable rate limits":

1. **OpenAI `text-embedding-3-small` at native 1536-d.** Already-shipped SDK, ~$0.02 per 1M tokens, public dimensionality reduction available, well-understood rate-limit shape. Best default for the focused design.
2. **OpenAI `text-embedding-3-small` at 512-d.** Same provider, smaller index, faster search. Default for very large code corpora where the recall hit is acceptable.
3. **Voyage `voyage-code-3` (or the current Voyage code-specialized successor).** Best quality-for-cost on Dataset C specifically; recommended config override when code dominates.
4. **OpenAI-compatible aggregator (Together / Fireworks / DeepInfra) hosting BGE-large or Nomic.** Cheapest, lowest vendor lock-in, smoothest path to fully-local later. Recommended fallback when the operator wants to avoid OpenAI.

Anthropic is explicitly excluded as an embeddings provider because no such API exists.
...
- A worker-thread offload point is reserved in the in-process design so local embedding can be added later without restructuring the request path.
...
- Runtime path: load into `better-sqlite3`. Do NOT design against `node:sqlite`. The Node built-in `node:sqlite` module landed as experimental later than the Node 20.x line Saivage targets, and is not a supported substrate here. `better-sqlite3` is the realistic primary: prebuilt native binary across common arches, fully synchronous TypeScript-typed API, plays well with the sync style of [saivage/src/knowledge/store.ts](saivage/src/knowledge/store.ts).
...
Tree-sitter friction under Node 20 ESM: the native `tree-sitter` Node bindings have historically required prebuilds and have had ESM-vs-CJS interop issues per language grammar. The portable Wasm path (`web-tree-sitter`) avoids the native build but is slower and has its own ESM packaging quirks. The fallback path is therefore architecturally first-class, not a hack:
...
3. For each re-chunked file, compute the new chunk set with per-chunk `contentHash`. Compute the symmetric difference against the store's existing chunks for that `(datasetId, projectId, path)`:
   ...
   - Existing rows whose `contentHash` is absent from the new chunk set → hard delete. This is what handles shifted chunk boundaries: even if `chunkIndex` is now stable, the old content at that index is gone, so the old row is gone.
4. Reuse an embedding cache keyed by `(provider, model, dim, revision, sha256(content))`. Cache hits skip the embedding API call. The cache lives in the same SQLite file as the chunks (e.g. a `chunk_keys` table) so it is per-project and trivially rebuilt by deleting the file.
...
Worked example of the shifted-boundary case:

- Before edit, a 1200-line TypeScript file at `src/foo.ts` produced 4 chunks: chunkIndex 0 (lines 1–320, contentHash `A`), 1 (321–640, `B`), 2 (641–960, `C`), 3 (961–1200, `D`). Store has 4 rows.
...
If the configured embedding provider, model, dimension, or revision differs from what is stamped on existing rows, those rows are unusable. Vector spaces do not commute across providers, models, dimensions, or even (in some cases) model revisions.

Required behavior:

- On open, compare the configured `(provider, model, dim)` against the values stamped on any existing row in the dataset.
- On mismatch: refuse queries with a clear, single error: "Embedding configuration changed (was `<old>`, now `<new>`); run `rebuild` to re-embed dataset `<datasetId>`." Do NOT auto-rebuild silently. Do NOT mix vectors. The implementer must not add a code path that retries against a different embedding configuration.
...
- **Generated and vendored files.** Source trees contain generated TypeScript (`*.d.ts` shadowing implementation, TypeDoc HTML/MD output), vendored libraries (`vendor/`, `third_party/`, `node_modules/` if it leaks past gitignore), and machine-built artifacts (`dist/`, `build/`, `out/`, `coverage/`). All are excluded by default at the ingest layer (§7.5); the chunker never sees them.
- **Binary or near-binary blobs.** Minified JS, source maps, base64-encoded payloads in JSON fixtures, lockfiles (`package-lock.json`, `pnpm-lock.yaml`, `poetry.lock`, `uv.lock`). Excluded by default. If something slips through, the secret-scan and entropy check at §7.6 catches high-entropy payloads and the chunker drops them with a logged reason.
...
- `dataset.register(name, config)` — declare a logical dataset and its provider/store/chunker configuration. Datasets not registered cannot be ingested or queried.

### 6.2 Ingest

- `dataset.ingest(documents | path glob)` — incremental ingestion. Caller passes either pre-resolved document objects (`{path, text, metadata}`) or a glob; the chunker registered for the dataset turns them into chunks; idempotency rules from §5.5 apply.
- `dataset.delete(ids | filter)` — remove chunks by id or by metadata filter (typical: `{path: ...}`).

### 6.3 Query

- `dataset.query(text | vector, topK, filter)` — embed `text` (or use the supplied raw `vector`), rank top-K against the dataset, apply the metadata filter, return the chunks with text, metadata, and score. The embedding role (query vs. document) is determined internally by the call site; it is NOT a caller-supplied knob.

### 6.4 Lifecycle

- `dataset.stats()` — chunk count, distinct file count, last-ingest timestamp, stamped `(provider, model, dim)`, on-disk byte size.
- `dataset.rebuild()` — drop and re-ingest. Required after embedding-configuration change.
- `dataset.drop()` — delete all chunks and on-disk artifacts for the dataset.
...
Provider 429 / 5xx responses must be retryable with exponential backoff and respect for `Retry-After` headers. The EXACT retry policy, batch size, jitter shape, and concurrency ceiling are NOT designed here — they are design-round work because they couple to the chosen provider's actual rate-limit shape (and the implementer must first run the VERIFY items in §2 before picking values).
...
On-disk RAG artifacts under `<projectRoot>/.saivage/rag/` inherit `.saivage/` directory permissions (operator-owned, mode 0700 directory / 0600 files where the OS supports them).
...
Provider API keys are read through Saivage's existing provider/config layer ([saivage/src/knowledge/store.ts](saivage/src/knowledge/store.ts) reads `.saivage/auth-profiles.json` via the secure boundary). The RAG layer:
...
- `<projectRoot>/.saivage/rag/` is safe to delete entirely; the subsystem rebuilds from source on the next ingest. The on-disk store is derived data, not source of truth. This is the canonical disaster-recovery path.
...
- **Corrupted .db on disk.** Power loss, disk-full mid-write, or external process truncates the sqlite-vec file. Expected behavior: startup `PRAGMA integrity_check` fails; the dataset is marked corrupted; queries return a clear error directing the operator to `rebuild`; ingest will not silently overwrite a corrupted file. Operator can also delete `<projectRoot>/.saivage/rag/<datasetId>.db` (§7.10) and re-ingest from source.
...
- The earlier claim that 1% incremental ingest is "sub-second under hosted embeddings" is wrong at this token volume. Sub-second is achievable only for very small incrementals (a handful of files) or when query-embedding cache hits dominate.
...
- Default vector store: sqlite-vec on top of better-sqlite3, single `.db` per dataset under `<projectRoot>/.saivage/rag/`. Brute-force exact KNN. No `node:sqlite`.
...
- VERIFY: whether [@mariozechner/pi-ai](saivage/package.json#L31-L32) exposes an embeddings surface, and for which providers, by inspecting the installed `saivage/node_modules/@mariozechner/pi-ai/dist`.
...
- VERIFY: `better-sqlite3` Node 20 ESM compatibility and prebuild availability for the operator's arch at [github.com/WiseLibs/better-sqlite3](https://github.com/WiseLibs/better-sqlite3).
- VERIFY: `@lancedb/lancedb` Node 20 ESM compatibility and prebuild availability at [github.com/lancedb/lancedb](https://github.com/lancedb/lancedb).
- VERIFY: `tree-sitter` Node 20 ESM compatibility for the chosen language grammars (`tree-sitter-typescript`, `tree-sitter-python`); confirm whether the native or Wasm path is the right primary.
