...
[src/rag/manager.ts](src/rag/manager.ts#L34-L40). `projectId` is
sourced from the existing `project_name` field on
`SaivageConfig` (the only project-id-shaped field on the current
config schema). The same `ragDatasets` array object is passed to
