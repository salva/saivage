# F01 — Markdown Table Rendering — Analysis r2

Round-2 revision of `01-analysis-r1.md`. All sections retained; reviewer items
(R2-1 … R2-8) addressed in-place. No code touched.
...
- VitePress docs site (`docs:dev`, `docs:build`, `docs:preview` —
  ...
  here, transitively via VitePress / typedoc (see §7); not bundled into
  `web/`.
...
```
$ npm ls markdown-it marked dompurify markdown-table micromark sanitize-html --all
```

results — and cross-checked against `package-lock.json` `node_modules/<pkg>`
blocks:

- **`markdown-it@14.1.1`** —
  block at [package-lock.json#L8069-L8085](package-lock.json#L8069-L8085).
  Pulled in by:
  - **typedoc** (root devDependency
    [package.json#L66](package.json#L66) `"typedoc": "^0.28.3"`; lock
    block [package-lock.json#L11426-L11447](package-lock.json#L11426-L11447);
    declares `"markdown-it": "^14.1.1"` at
    [package-lock.json#L11434](package-lock.json#L11434)). Visible to
    `npm ls --all` as the single resolved parent.
  - **vitepress** (root devDependency
    [package.json#L68](package.json#L68) `"vitepress": "^1.6.4"`; lock
    block at [package-lock.json#L12219](package-lock.json#L12219)).
    Vitepress's own `package.json` lists `"markdown-it": "^14.1.0"`
    plus a constellation of `markdown-it-anchor`, `markdown-it-attrs`,
    `markdown-it-container`, `markdown-it-emoji`, `markdown-it-mathjax3`
    (verified by inspecting
    `node_modules/vitepress/package.json` lines 120-125 in the local
    install). It does not surface in `npm ls --all` because npm dedupes
    it under typedoc, but if typedoc is removed VitePress still drags
    `markdown-it` into the lockfile.

  Neither path reaches the web bundle: typedoc runs at docs API
...
- **`marked@16.4.2`** —
  block at [package-lock.json#L8098-L8109](package-lock.json#L8098-L8109).
  Sole parent is **`mermaid@11.15.0`**
  ([package-lock.json#L8362-L8390](package-lock.json#L8362-L8390); declares
  `"marked": "^16.3.0"` at
  [package-lock.json#L8386](package-lock.json#L8386)). `mermaid` itself
  is pulled in by the dev dep `vitepress-plugin-mermaid`
  ([package.json#L69](package.json#L69)), used only by the VitePress
  docs build.

- **`dompurify@3.4.5`** —
  block at [package-lock.json#L6051-L6058](package-lock.json#L6051-L6058).
  Sole parent is **`mermaid@11.15.0`** (declares `"dompurify": "^3.3.1"`
  at [package-lock.json#L8381](package-lock.json#L8381)). Same docs-only
  path as `marked`.
...
1. **Extend or replace?** Should table support be a new rule appended to
   ...
   §3?
2. **Sanitizer / HTML-disabled configuration.** (R2-8: reformulated.)
   Should the web renderer remain dependency-free or add an explicit
   web dependency, and **which sanitizer / HTML-disabled configuration
   would preserve the current escape-first XSS posture** documented in
   §5? Concretely: keep the escape-first regex shape and add only a
   table rule; introduce a parser with raw-HTML disabled (`markdown-it`
   `html: false`, `marked` `mangle:false, headerIds:false` plus no
   `<script>`/`<style>` emission); or introduce a parser plus an
   explicit post-pass through DOMPurify / sanitize-html. Each option
   must keep the property that no user-supplied HTML reaches `v-html`.
