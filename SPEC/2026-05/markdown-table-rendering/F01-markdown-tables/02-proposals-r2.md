# F01 — Markdown Table Rendering — Proposals r2

Round-2 revision of
[02-proposals-r1.md](02-proposals-r1.md), addressing the six items in
[02-proposals-review-r1.md](02-proposals-review-r1.md). Structure
(sections A–G) is preserved from r1; only the reviewer-flagged claims are
adjusted, plus the §F policy-compliance paragraph required by the
workspace clean-architecture rule. Every code claim cites either the
...
### Table block detection (narrow scope, per review item 3)

A table block is a contiguous run of lines that, after the escape step,
matches all three:

1. **Header row:** `/^\s*\|.+\|\s*$/m` — leading **and** trailing `|`
   required; at least one `|` interior.
...
3. **Zero or more body rows:** same `^\s*\|.+\|\s*$` shape (leading +
   trailing `|`), terminated by either a blank line, EOF, or a line that
   does not match.

#### Known-unsupported inputs within Proposal A's scope

Proposal A's table support is deliberately narrower than `marked`'s GFM
table grammar. The following inputs are *not* covered by A and produce
either no table or visibly wrong output, by construction:

- **Tables without leading/trailing pipes.** `marked` (`gfm: true`)
  accepts `a | b\n---|---\n1 | 2` as a valid table; Proposal A's
  `^\s*\|.+\|\s*$/m` rejects it and the rows fall through to the inline
  pipeline as plain text.
- **Escaped pipes `\|` inside cells.** Proposal A splits cells on
  unescaped `|` only by literal regex; backslash-escaped pipes would
  require a separate tokenizer pass. Inputs containing `\|` will be
  split at the wrong boundary and emit garbled cells.
- **Ragged body rows.** When a body row has fewer or more cells than the
  header, Proposal A's behaviour is undefined within this proposal
  (implementation may drop, truncate, or pad — the proposal does not
  pick); `marked` pads short rows with empty `<td>` and ignores extra
  cells.
- **Empty compact cells `||`.** Proposal A's cell split treats a run of
  `||` as a single empty boundary or as two pipes depending on greediness
  of the chosen regex; the proposal does not commit to either. `marked`
  emits an empty `<td>` per missing cell.

These limitations are intrinsic to a regex-only block tokenizer; closing
them effectively rebuilds the relevant slice of `marked`'s table grammar
in hand-rolled regex, which is the policy-cost flagged in §F below.
...
- **Fix in this proposal:** tables (the trigger, with the narrow scope
  enumerated above); italics alone
...
- **Workspace policy bias is against this direction.**
  ...
  grows it instead. See §F for the policy-compliance assessment.
...
| ---------------------------------------- | ----- | ---- |
| 1. GFM table fidelity                    | 2     | Header/body and alignment fine for fully-piped, simple-content tables; explicitly does NOT cover pipe-less tables, escaped pipes `\|`, ragged body rows (behaviour undefined), or empty `||` cells — see "Known-unsupported inputs" above. |
...
`marked`'s relevant option flags are declared in `MarkedOptions`: `gfm?:
boolean` at
...
and `async?: boolean` at
[node_modules/marked/lib/marked.d.ts#L441](../../../../node_modules/marked/lib/marked.d.ts#L441).
The synchronous `parse` overload (returns `string` when `async: false`)
lives on the `Marked` class at
[node_modules/marked/lib/marked.d.ts#L623-L635](../../../../node_modules/marked/lib/marked.d.ts#L623-L635);
the exported top-level `marked(text, opts)` synchronous overload (also
returning `string` when `async: false`) is at
[node_modules/marked/lib/marked.d.ts#L675-L692](../../../../node_modules/marked/lib/marked.d.ts#L675-L692).
The relevant call shape is therefore synchronous, GFM on, soft-line-breaks
on.
...
therefore mandatory, not optional. The `marked.parse` overload at
[marked.d.ts#L623-L635](../../../../node_modules/marked/lib/marked.d.ts#L623-L635)
with `async: false` (see `async?` flag at
[marked.d.ts#L441](../../../../node_modules/marked/lib/marked.d.ts#L441))
...
#### DOMPurify hardening note (per review item 5)

Implementation must **not** broaden the sanitizer beyond `USE_PROFILES: {
html: true }`. In particular, the implementation PR must not introduce
custom `ADD_TAGS`, `ADD_ATTR`, `ALLOWED_URI_REGEXP` relaxations, or
non-default protocol allowances without a separate security review.
Default DOMPurify already permits `table` / `thead` / `tbody` / `tr` /
`th` / `td` plus the `align` attribute — which is exactly what `marked`
emits for GFM tables with alignment — so the default profile is
sufficient for this proposal's goals. Any future requirement that would
require relaxing the profile (e.g. embedding `<iframe>` or extra URI
schemes) must be raised as a new spec under
`SPEC/.../sanitizer-policy/`, not slipped into this F01 implementation.
...
- **Closes most of the §3 gap list wholesale** — italics, links,
  autolinks, images (sanitized), blockquotes, thematic breaks,
  strikethrough, task-list checkboxes, hard breaks, nested lists — all
  become "what GFM says". Footnote rendering is **not** included by
  default `marked@16.4.2` and is deliberately out of scope here (see §G
...
| Criterion                                | Score | Note |
| ---------------------------------------- | ----- | ---- |
| 1. GFM table fidelity                    | 5     | `gfm: true` covers header, body, and alignment per [marked.d.ts#L453](../../../../node_modules/marked/lib/marked.d.ts#L453). |
| 2. Coverage of other §3 gaps             | 4     | Closes 8 of 9 §3 gaps (italics, links/autolinks, blockquotes, thematic breaks, strikethrough, task-list checkboxes, hard breaks, nested lists). Footnotes are not rendered by default `marked@16.4.2` and are out of scope — see §G. |
| 3. Bundle-size impact                    | 2     | Expected ~55 KB pre-gzip; to be measured per §7. |
| 4. XSS safety                            | 5     | DOMPurify whitelist on parsed DOM is stronger than escape-first heuristic; profile pinned to `USE_PROFILES: { html: true }` (see hardening note above). |
...
| --- | --- | --- |
| 1. GFM table fidelity (3) | 2 × 3 = **6** | 5 × 3 = **15** |
| 2. Coverage of other §3 gaps (3) | 2 × 3 = **6** | 4 × 3 = **12** |
...
are on Cleanness (#8: −3), GFM table fidelity (#1: −3), and §3 gap
coverage (#2: −2) — i.e. on the dimensions that prompted the work and
the dimension the workspace policy explicitly privileges. The r2 score
adjustments (A's #1 from 3→2 per review item 3, B's #2 from 5→4 per
review item 4) shrink B's lead by 0 and 3 points respectively, but the
ordering and recommendation are unchanged.
...
1. **GFM tables are the trigger.** Proposal B turns OQ1
   ...
   (`gfm: true`,
   [node_modules/marked/lib/marked.d.ts#L453](../../../../node_modules/marked/lib/marked.d.ts#L453)).
   Proposal A scores 2/5 on the same axis because its hand-rolled
   tokenizer is documented as not handling pipe-less tables, escaped
   pipes, ragged rows, or empty `||` cells (see §C "Known-unsupported
   inputs") — exactly the inputs that `marked`'s GFM grammar handles by
   construction.
2. **The §3 gap list disappears (modulo footnotes).** Operator-facing
   content has had silently-dropped italics, links/autolinks,
   blockquotes, strikethrough, task-list checkboxes, and nested lists
   for the renderer's entire lifetime
   ([01-analysis-r3.md#§3](01-analysis-r3.md)). Proposal A closes 3–4 of
   those; Proposal B closes 8 of the 9 (footnotes excluded — see §G).
   For the *same* amount of consumer-side CSS churn (both proposals touch
   the same `:deep(...)` blocks in
...
4. **The XSS posture improves, not regresses.** DOMPurify on the parsed
   ...
   removes that fragility, provided the profile is pinned per the
   hardening note in §D.
...
### Workspace policy compliance (per review item, r2)

The reviewer correctly flagged that Proposals A and B are **not
policy-equal**. The workspace rule
([01-analysis-r3.md#§8b](01-analysis-r3.md), and the project-wide
"architecture-first / no backward-compatibility" rule that applies to
saivage, saivage-v3, getrich, getrich-v2) directs implementers to
*remove* hand-rolled code paths in favour of clean architecture, not to
keep them and grow them. Proposal A is therefore **policy-non-compliant
as a final design choice**: it adds ~80 LOC of new block-tokenizer plus
inline rules to
[web/src/utils/markdown.ts](../../../../web/src/utils/markdown.ts#L1-L29)
while deleting 0 LOC of the existing pipeline, which is the opposite of
what the policy asks for. Proposal A is documented here as a **rejected
alternative** (kept for traceability and to make the trade-offs visible
in the comparison table), but it must not be selected as the
implementation path. The recommendation is **Proposal B**.
...
- **OQ2 (sanitizer config)** — explicit `DOMPurify.sanitize(..., {
  ...
  Profile pinning per the §D hardening note is mandatory.
...
- [web/src/components/JsonHighlight.vue](../../../../web/src/components/JsonHighlight.vue)
  ...
  — the JSON-vs-text branching contract (the `<JsonHighlight v-if=...>`
  branch opens at L68) is preserved as-is
...
- **User and system chat messages remain non-markdown.** `renderMarkdown`
  continues to apply **only** to assistant messages — cite the role
  branch at
  [web/src/components/ChatWindow.vue#L314](../../../../web/src/components/ChatWindow.vue#L314)
  (`v-if="msg.role === 'assistant'"` selects `v-html="renderMarkdown(...)"`;
  the `v-else` branch on the next line keeps `{{ msg.content }}` as plain
  text). This proposal does not change that role gating.
- **No changes to chat history storage, WebSocket transport payloads, or
  backend agent output formatting.** Markdown rendering is a presentation
  concern owned entirely by the `web/` client; the wire format and
  on-disk shapes of agent messages are left untouched.
...
- **DOMPurify profile / URL-scheme allowlist broadening** (per review
  ...
  allowances; the sanitizer call is pinned to `USE_PROFILES: { html:
  true }`. Default DOMPurify already permits `table` / `thead` / `tbody`
  / `tr` / `th` / `td` plus the `align` attribute that `marked` emits
  for aligned tables, so the default profile is sufficient. Any future
  profile relaxation requires a separate security review under
  `SPEC/.../sanitizer-policy/`.
- **Image handling beyond `marked`'s default sanitized `<img>` rendering.**
  No lazy-loading attributes, no width/height constraints, no domain
  allowlist on `src`, no thumbnail proxying. `marked` emits a plain
  `<img>`; DOMPurify strips `on*` handlers and non-allowed schemes via
  the default html profile. Anything beyond that (e.g. CSP-aware image
  loading, hot-link prevention, lazy decode) is a separate concern.
