...
| NB-5: compaction sufficiency | ACCEPT-FIX | CONFIRMED | §5.12 directly compares memory against `plan-history.json`, `plan.json`, and the compaction summary, with options and a boundary-pinning regression test in [00-FUNCTIONAL-ANALYSIS.md](00-FUNCTIONAL-ANALYSIS.md#L1115-L1142). |
...
NB-5 asked whether compaction plus `plan.json` / `plan-history.json` is already sufficient, and what memory adds beyond those existing recovery channels. §5.12 matches this intent cleanly: it compares history-only, memory-only, and dual-read options and requires a regression test to pin the boundary.
