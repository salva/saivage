...
- Corrected Proposal A's risk section: the web UI route is `/api/state` (not `/api/status`). Verified against [src/server/server.ts](src/server/server.ts#L179) (`app.get("/api/state", async () => {`) and [web/src/components/StatusPanel.vue](web/src/components/StatusPanel.vue#L58) (`apiFetch("/api/state")`). No other endpoint named `/api/status` exists in the repo.
...
- Web UI is unaffected: `/api/state` reads `paths.runtimeState`, not the mirror.
