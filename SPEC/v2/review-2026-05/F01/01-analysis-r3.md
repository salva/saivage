...
- **Accepted (reviewer required change #1):** fixed the broken operator-note link under "Operator constraint". It previously read `[F01-designer-agent-orphan.md L25-L27](../../F01-designer-agent-orphan.md#L25-L27)`, which from the `F01/` directory resolved to the non-existent `SPEC/v2/F01-designer-agent-orphan.md`. The issue file actually lives one level up at [F01-designer-agent-orphan.md](../F01-designer-agent-orphan.md), so the link is now `../F01-designer-agent-orphan.md#L25-L27`.
- Re-verified every other clickable reference in this file against the live tree from the `F01/` directory: the `../../../../src/...` paths resolve to `/home/salva/g/ml/saivage/src/...` (four levels up: `F01/` -> `review-2026-05/` -> `v2/` -> `SPEC/` -> repo root), the `../F09/...` paths resolve inside the same review directory, and `../_LOOP-CONVENTIONS.md` resolves at the review root. No other path errors found.
- No factual claims changed; the orphan evidence, F09 entanglement, contract table, call-site survey, and constraints section all carry forward from r2 unchanged in substance.
...
- **Not in the Manager prompt's valid-values sentence.** [src/agents/manager.ts L260](../../../../src/agents/manager.ts#L260) tells the Manager that valid `type` values are `code, research, data, review, test, document` and valid `assigned_to` values are `coder, researcher, data_agent, reviewer` — neither `design` nor `designer` appears. Even if every Zod schema and routing table were widened, the Manager would never emit a `design`/`designer` task because its own system prompt forbids it.
...
| Manager prompt — worker roster | [src/agents/manager.ts L29-L51](../../../../src/agents/manager.ts#L29-L51) (roster narrative) and [manager.ts L77-L116](../../../../src/agents/manager.ts#L77-L116) (tool reference block) | add a Designer paragraph + `run_designer({ task, stageId })` reference |
| Manager prompt — valid-values sentence | [src/agents/manager.ts L260](../../../../src/agents/manager.ts#L260) | add `design` to the valid `type` list and `designer` to the valid `assigned_to` list |
...
This roster of touch sites is itself evidence that the role was never wired — twelve independent surfaces all omit it consistently (one more than r1, after adding the Manager valid-values sentence).
