# F01 — Implementation plan (R2)

## Changes from r1

- **Accepted (reviewer required change #2):** Step 9 now declares the new `DesignerAgent` constructor with `config?: Partial<BaseAgentConfig>` (imported from `./base.js`) instead of `config?: Partial<WorkerAgentConfig>`. This matches F09's standard worker subclass shape ([F09/02-design-r2.md L286-L294](../F09/02-design-r2.md#L286-L294)) and prevents external callers from overriding worker invariants (`role`, `systemPrompt`, `buildInitialMessage`, `invalidFinalResponseMessage`). The import line in the snippet changes from `import { WorkerAgent, type WorkerAgentConfig } from "./worker.js";` to `import { WorkerAgent } from "./worker.js";` plus `import type { BaseAgentConfig } from "./base.js";`. The bootstrap spawner call site in Step 8 only passes `onActivity`, which is a `BaseAgentConfig` field, so no caller-side change is needed.
- **Accepted (reviewer required change #3):** Step 11 now explicitly updates the Manager prompt's valid-values sentence at [src/agents/manager.ts L260](../../../../src/agents/manager.ts#L260), adding `design` to the `type` list and `designer` to the `assigned_to` list. r1 only listed L29-L51 / L77-L116 / L335.
- The pre-condition, file count (1 added + 10 edited + 1 test edited), and ordering remain unchanged. Steps 1-7, 9 (apart from the constructor signature & imports), and 10-12 are otherwise identical to r1.

Plan for the **recommended Proposal C** from [02-design-r2.md](02-design-r2.md): land F09 first, then land F01 as a single follow-up commit that creates a fresh `DesignerAgent extends WorkerAgent` and wires it into every role surface.
...
8. **Add bootstrap spawner case** in [src/server/bootstrap.ts L290-L367](../../../../src/server/bootstrap.ts#L290-L367) `createChildSpawner`'s `switch (role)`:
   ...
   Insert between the `data_agent` case ([bootstrap.ts L325-L333](../../../../src/server/bootstrap.ts#L325-L333)) and the `reviewer` case. Add `import { DesignerAgent } from "../agents/designer.js";` at the top of the file alongside the other agent imports. `onActivity` is a `BaseAgentConfig` field, so this call is compatible with the narrower `Partial<BaseAgentConfig>` constructor signature established in Step 9.
...
9. **Create the new agent file** at `src/agents/designer.ts`:
   ...
   The constructor accepts `Partial<BaseAgentConfig>` (not `Partial<WorkerAgentConfig>`) so the four worker-invariant fields (`role`, `systemPrompt`, `buildInitialMessage`, `invalidFinalResponseMessage`) cannot be overridden by callers. This matches the public shape F09 establishes for every concrete worker subclass at [F09/02-design-r2.md L286-L294](../F09/02-design-r2.md#L286-L294). The `super(ctx, input, {...})` call still passes a full `WorkerAgentConfig` because the worker-invariant fields are set positionally inside the subclass; only `BaseAgentConfig`-shaped knobs (e.g. `onActivity`) flow through `...config`.
...
11. **Update manager prompt** in [src/agents/manager.ts](../../../../src/agents/manager.ts) — three distinct surfaces:
    - **Worker roster narrative** at [L29-L51](../../../../src/agents/manager.ts#L29-L51): add a Designer paragraph to the worker introductions explaining that `run_designer({ task, stageId })` dispatches design tasks (briefs, UX flows, architecture decisions) and that it produces design artifacts but does not write production code.
    - **Tool reference block** at [L77-L116](../../../../src/agents/manager.ts#L77-L116): add a `\`run_designer({ task, stageId })\` — Dispatch a design task to a Designer agent. ...` entry in the same format as the other workers.
    - **Valid-values sentence** at [L260](../../../../src/agents/manager.ts#L260): the current text is `Valid \`type\` values are \`code\`, \`research\`, \`data\`, \`review\`, \`test\`, \`document\`. Valid \`assigned_to\` values are \`coder\`, \`researcher\`, \`data_agent\`, \`reviewer\`.`. Change to `Valid \`type\` values are \`code\`, \`research\`, \`data\`, \`review\`, \`test\`, \`document\`, \`design\`. Valid \`assigned_to\` values are \`coder\`, \`researcher\`, \`data_agent\`, \`reviewer\`, \`designer\`.`. Without this, the Manager will continue to emit only the legacy four `assigned_to` values even though every Zod schema and tool registration accepts `designer` — the prompt itself would forbid what the system now allows.
    - **Dispatch guard** at [L335](../../../../src/agents/manager.ts#L335) `hasUsedToolNamed(...)`: add `"run_designer"` to the list so a Manager that dispatched only a Designer is not flagged as having dispatched nothing.
...
1. `npm run typecheck` — must pass. The strict-mode TypeScript compile will catch every missed enum / role-keyed record (this is the main protection that all 12 surfaces were updated).
...
- The twelve wiring surfaces revert to "no `designer`" (which is the post-F09 state).
