...
- Fixed memory/skill row-count wording in the "Constraints" section: "the four memory/skill rows in `cmdHelp`" was wrong — the table has seven rows at [src/agents/chat.ts](src/agents/chat.ts#L379-L385). Updated to "seven memory/skill rows".
- Fixed an unrelated count typo in the "Contract" section: "Five of nine handlers ignore `args`; four (...) consume it" was wrong both ways — three ignore (`/help`, `/status`, `/plan`) and six consume (`/history`, `/replan`, `/restart-planner`, `/note`, `/note!`, `/notep`).
- All other content carried over unchanged from r1; the underlying problem, contract, call-site, and constraint analysis was correct in r1.
...
- **Per-command handler shape**: `(args: string) => string | Promise<string>`. Three of nine handlers ignore `args` (`/help`, `/status`, `/plan`); six consume it (`/history`, `/replan`, `/restart-planner`, `/note`, `/note!`, `/notep`).
...
1. **Out of scope, do not cross**: `src/skills/`, `src/chat/slashCommands.ts`, `parseSlashCommand`/`runSlashCommand`, and the seven memory/skill rows in `cmdHelp`. Per loop convention §Out-of-scope and the F30 prompt scope note.
...
10. **Dispatch must be structurally tied to the registry** (added in r2): replacing the `switch (cmd)` with a different `switch (c.name)` over the same hand-listed command names is not acceptable — it just moves the duplication. The dispatch lookup must be derived from `LOCAL_CHAT_COMMANDS` such that adding or removing a registry entry without a matching handler edit is a TypeScript compile error, not merely a test failure.
