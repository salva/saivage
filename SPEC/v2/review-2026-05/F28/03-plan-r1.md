# F28 — Plan r1 — implementation steps for Proposal B
...
1. **Create** [src/mcp/types.ts](src/mcp/types.ts) with two interfaces, no Zod:
   ...
   ```
   Note: the `status` field and the `"generated"` origin variant are dropped (no remaining producer or consumer — see [SPEC/v2/review-2026-05/F28/01-analysis-r1.md](SPEC/v2/review-2026-05/F28/01-analysis-r1.md)).
...
9. **Update SPEC of record:**
   - [SPEC/v2/05-MCP-SERVICES.md](SPEC/v2/05-MCP-SERVICES.md#L412): strike the bullet that references `registry.json` for `origin: "generated"` entries; the v1 MCP-generator feature is not present in v2.
...
None. Every deleted code path was unreachable in the dispatcher (no producer for the registry entries), so there is no observable surface to assert. Adding a "throws when service not running" assertion for the new `startService` error message is justifiable but not load-bearing; if added, place it in [src/mcp/runtime.test.ts](src/mcp/runtime.test.ts):

```ts
it("throws a config-pointing error when a service is not running", async () => {
...
This is the only new test worth adding; it pins the error message that replaces the now-removed registry-lookup branch.
...
5. Manual sanity: `grep -rn "registry\.json\|registerService\|unregisterService\|listRegisteredServices\|updateServiceStatus\|getService\b" src/ tests/ web/` must return zero matches.
6. Manual sanity: `ls .saivage/registry.json 2>/dev/null` in any active project (e.g. on the `saivage-v3` container) must return nothing — the file was never created, and after this change there is no code that would create it.

## Rollback strategy

Single squash-merged commit. Revert via `git revert <sha>`. No on-disk state is created or migrated by this change; reverting restores the dead code without any data recovery step.
