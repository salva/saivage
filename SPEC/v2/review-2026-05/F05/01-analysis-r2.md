# F05 — Functional analysis (R2)

## Changes from r1

- **Accepted (reviewer required change 1).** Every `src/runtime/supervisor.ts` line anchor has been re-verified against the current file (`wc -l src/runtime/supervisor.ts` → 272). Corrected anchors: `askModel` is L125-L147 (was L122-L156); the `getRecentLogs(this.logLines)` block is L126-L128 (was L123-L125); the normalizer call site is L146 (was L154); `maxTokens: 600` is L143 (was L153); `SUPERVISOR_SYSTEM_PROMPT` is L161-L168 (was L158-L168); `parseVerdict` is L176-L202 (was L171-L201); `parseJsonObject` is L204-L220 (was L204-L218); `normalizeNonStuckOperationalVerdict` body is L222-L257 (was L220-L257); the three `looksLike*` predicates are now individually addressable: `looksLikeLongRunningExternalWork` L259-L264, `looksLikeProviderThrottling` L266-L268, `looksLikeMalformedOrCrashed` L270-L272 (was the lumped L246-L257). No substantive content change.
- **Rejected:** none.

## Problem restated

`RuntimeSupervisor.askModel` ([src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L125-L147)) does three things in order:

1. Pulls recent log entries via `getRecentLogs(this.logLines)` ([src/log.ts](../../../../src/log.ts#L35)) and joins their `formatted` lines into one blob ([src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L126-L128)).
2. Sends them to the supervisor LLM with a system prompt that explicitly enumerates the rules "treat throttling/rate-limit/quota/429/overload as stuck=false" and "treat long-running shell/training/benchmark/build/test/playwright work as stuck=false" ([src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L161-L168)).
3. Parses the model reply into a `SupervisorVerdict`, then **runs `normalizeNonStuckOperationalVerdict(verdict, logs)`** ([src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L146)) which can flip `stuck=true` back to `stuck=false` based on regex heuristics over the verdict text and the log blob.

The post-processor body ([src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L222-L257)) tries three regex predicates in two passes:
...
- If `looksLikeMalformedOrCrashed(value)` matches (`Unterminated string|Unexpected end of JSON|malformed|No tool call found|orphaned tool|context_length_exceeded|exceeds the context window|unhandled exception|unhandled rejection|TypeError|ReferenceError|SyntaxError|crash|failed to parse`, [src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L270-L272)) → leave `stuck=true`.
- Else if `looksLikeLongRunningExternalWork(value)` matches ([src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L259-L264)) → flip to `stuck=false` with a synthesised reason "Long-running external work is not itself stuck.".
- Else if `looksLikeProviderThrottling(value)` matches (`rate-limit(ed|ing)?|throttl(ed|ing)|too many requests|\b429\b|quota|temporar(y|ily) unavailable|capacity|overloaded`, [src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L266-L268)) → flip to `stuck=false` with reason "Provider throttling/rate limiting is temporary; not treating as stuck.".

The two passes are not equivalent: Pass 2 widens the input by the entire 400-line log blob (`DEFAULT_LOG_LINES=400`, [src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L11)). So any earlier unrelated log line that contains the substring "throttl" or "capacity" or "training" or "running" will satisfy the predicate on Pass 2 even when the verdict itself is talking about something else.

## Why this is a bug, not a safety net

The system prompt at [supervisor.ts L161-L168](../../../../src/runtime/supervisor.ts#L161-L168) already instructs the LLM:
...
The supervisor's own counter `consecutiveStuckVerdicts` ([supervisor.ts L10](../../../../src/runtime/supervisor.ts#L10), default `3`) compounds the problem: the flip resets the counter via the `if (!verdict.stuck)` branch at [supervisor.ts L82-L88](../../../../src/runtime/supervisor.ts#L82-L88). One flipped verdict undoes two prior real-stuck verdicts.

## Contract

The supervisor produces one of two observable effects per `checkOnce()` tick:

- Increments `consecutiveStuck`. When it reaches `threshold` (`config.supervisor.consecutiveStuckVerdicts ?? 3`), it picks an agent per `ROLE_ABORT_PRIORITY` ([supervisor.ts L14](../../../../src/runtime/supervisor.ts#L14)) and calls `target.agent.cancel()` ([supervisor.ts L103](../../../../src/runtime/supervisor.ts#L103)) and schedules a second `.cancel()` after `FORCE_CANCEL_DELAY_MS = 600_000` ms ([supervisor.ts L12](../../../../src/runtime/supervisor.ts#L12), [supervisor.ts L113](../../../../src/runtime/supervisor.ts#L113)).
...
`parseVerdict` ([supervisor.ts L176-L202](../../../../src/runtime/supervisor.ts#L176-L202)) defends against non-JSON via the F03-identified private `parseJsonObject` helper ([supervisor.ts L204-L220](../../../../src/runtime/supervisor.ts#L204-L220)). On any non-object content it synthesises a `stuck=true, confidence=0.4` verdict using the raw content as evidence — this is the supervisor's only "escalate on parse failure" path.

## Call sites & dependencies

- Producer of the post-processor's input:
  - `parseVerdict(response.content, provider)` ([supervisor.ts L146](../../../../src/runtime/supervisor.ts#L146)).
  - `getRecentLogs(this.logLines)` ([supervisor.ts L126-L128](../../../../src/runtime/supervisor.ts#L126-L128)).
...
- `normalizeNonStuckOperationalVerdict` has zero callers outside `supervisor.ts` (`rg -n 'normalizeNonStuckOperationalVerdict' src` → only the function definition at L222 and its single call site at L146).
...
- Tests that exercise the post-processor:
  - [src/runtime/runtime.test.ts](../../../../src/runtime/runtime.test.ts#L198-L224) — "does not cancel agents when the only reported problem is provider throttling": feeds the LLM verdict `{stuck: true, reason: "GitHub Copilot is returning 429 rate limit responses", evidence: ["provider throttling"]}` and asserts `cancel` was not called. **The verdict comes in as `stuck=true`; only the post-processor flips it.** This test exists specifically to lock in the current behaviour.
  - [src/runtime/runtime.test.ts](../../../../src/runtime/runtime.test.ts#L226-L252) — long-running shell command variant of the same pattern.
  - [src/runtime/runtime.test.ts](../../../../src/runtime/runtime.test.ts#L117-L148) — happy-path `stuck=true, reason: "retry loop"`: verdict is plain enough that no `looksLike*` regex matches and the counter increments normally.
  - [src/runtime/runtime.test.ts](../../../../src/runtime/runtime.test.ts#L149-L172) — three consecutive `stuck=true` verdicts cause cancel. Same pattern.
...
- F20 territory: `maxTokens: 600` ([supervisor.ts L143](../../../../src/runtime/supervisor.ts#L143)) is the only token budget in `askModel`. Sufficient for the small JSON verdict today and after F05.
...
- **Tests must follow the new contract**: the two regex-validation tests at [runtime.test.ts L198-L252](../../../../src/runtime/runtime.test.ts#L198-L252) are testing the wrong contract (they assert the supervisor overrides the LLM). They must be rewritten to test the right contract: when the LLM returns `stuck=false` for throttling/long-running scenarios, the supervisor does not cancel.
- **No emojis** anywhere.
