# F14 — Implementation plan (R2)

## Changes from r1

- Corrected stale BaseAgent line references throughout. Push at [src/agents/base.ts](../../../../src/agents/base.ts#L266), return at [src/agents/base.ts](../../../../src/agents/base.ts#L283), `protected messages` at [src/agents/base.ts](../../../../src/agents/base.ts#L135), `pushMessage` at [src/agents/base.ts](../../../../src/agents/base.ts#L718).
- Rewrote the reviewer regression-test fixture. The stub now performs a valid tool-use turn before the final no-tool response, because `ReviewerAgent.validateFinalResponse()` at [src/agents/reviewer.ts](../../../../src/agents/reviewer.ts#L137-L140) rejects any final response that arrives before at least one tool has been used; the r1 fixture would have driven the model-repair path instead of a successful review.
- Replaced direct `reviewer.messages` / `planner.messages` assertions with an inspection strategy that does not violate `protected` visibility. The tests assert on the contents of the **next** stubbed `router.chat()` request's `messages` array (the existing pattern in [src/agents/agents.test.ts](../../../../src/agents/agents.test.ts#L91-L145)), which is the exact `this.messages` snapshot the provider sees on its next call. For the single-call reviewer scenario where there is no "next call", the test drives a second `review(input)` to force a follow-up provider call that exposes the post-state.
- Corrected the planner test fixture. The success branch is keyed on the regex `/^\s*PLAN_COMPLETE\s*$/m` at [src/agents/planner.ts](../../../../src/agents/planner.ts#L216-L217), so the second stubbed response must be text matching `PLAN_COMPLETE`, not `plan_complete()`. The plan also no longer claims [src/agents/agents.test.ts](../../../../src/agents/agents.test.ts) already exercises `PlannerAgent` — it does not import or construct `PlannerAgent` today. The planner test goes into a new sibling file `src/agents/planner.nudge.test.ts`.
- Replaced the verification grep with an `rg` invocation that uses real regex whitespace.
...
The workspace skill at [.github/skills/saivage-development-validation/SKILL.md](../../../../../.github/skills/saivage-development-validation/SKILL.md) is **not applicable**: its commands target `/home/salva/g/ml/saivage-v3` and Jest, while F14 modifies `/home/salva/g/ml/saivage` (this repo), which uses Vitest (`"test": "vitest run"`, [package.json](../../../../package.json#L17)) and `tsc --noEmit` for typecheck ([package.json](../../../../package.json#L20)).

Repo-local validation:

1. `npm run typecheck` — must pass cleanly ([package.json](../../../../package.json#L20)).
2. `npm run lint` — must pass cleanly ([package.json](../../../../package.json#L19)).
