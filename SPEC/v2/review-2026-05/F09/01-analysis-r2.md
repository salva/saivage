...
Each copy hardcodes its own `agent: "coder" | "researcher" | "data_agent" | "reviewer" | "designer"` literal in both the parsed-success path and the fallback path. `"designer"` again is outside the `TaskReportSchema.agent` enum at [types.ts L164](../../../../src/types.ts#L164).
