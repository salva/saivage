# G43 — Analysis r2 (`planning` built-in skill teaches a fictional plan format)

Round-2 delta over [01-analysis-r1.md](./01-analysis-r1.md): Section 5 ("Why dormant") is rewritten to match the production resolver behaviour
(reviewer change 1 in [04-review-r1.md](./04-review-r1.md)). Sections 1–4, 6, 7 are unchanged in substance; this document repeats them in full so it stands alone.

## 1. What G43 reports vs. what the file says

The filed evidence at [../G43-planning-skill-fictional-plan-format.md](../G43-planning-skill-fictional-plan-format.md)
...
does not strip the YAML preamble, so any agent that *did* receive the
skill would literally see the `---`-fenced frontmatter as Markdown too.
Verbatim:
...
Whether that body is actually injected into a live planner turn is a
separate question — answered in Section 5.
...
no `summary` —
[src/types.ts](../../../../src/types.ts#L35-L44). The field name
`summary` IS used elsewhere — the `plan_complete_stage` MCP tool takes
a `summary` argument
([prompts/planner.md](../../../../prompts/planner.md#L65)), and both
`CompletedStage` and `StageSummary` carry a `summary` field
([src/types.ts](../../../../src/types.ts#L54-L62),
[src/types.ts](../../../../src/types.ts#L180-L183)) — but it is a
per-stage retrospective written by the Manager *after* a stage runs,
never a plan-document field set up-front by the planner. Validation
sentinels in [03-plan-r2.md](./03-plan-r2.md) must not key off the bare
word `summary` for that reason.
...
*if* the body is delivered at all (Section 5 shows it is not). G43 is
not the right place to fix that; G42 owns the loader fix.
...
## 5. Why the bug is currently dormant (revised)

Round-1 claimed the bug was dormant because the planner's system prompt
contradicts the skill body louder than the body asserts it. That is not
correct. The body never reaches the planner at all in production
today, for a chain of three independent reasons in the eager-load
pipeline:

1. **The walker synthesises a non-matching trigger.** For every
   built-in topic, `walkBuiltinSkills` constructs a `SkillRecord` with
   `triggers: [topic]` (here: `["planning"]`), `target_agents: []`,
   and `survive_compaction: false` —
   [src/knowledge/eagerLoader.ts](../../../../src/knowledge/eagerLoader.ts#L103-L118).
   The raw frontmatter on disk is ignored by the walker; the trigger
   list it actually emits is the bare topic name with no `kind:`
   prefix.

2. **The trigger scorer rejects bare triggers.** `scoreSkillTriggers`
   requires every trigger to have a `kind:` prefix (`keyword:`,
   `tag:`, or `agent:`); anything without a `:` is skipped silently
   ([src/knowledge/loader.ts](../../../../src/knowledge/loader.ts#L57-L58)).
   `"planning"` has no `:`, so it contributes zero score regardless of
   the planner's task description.

3. **The resolver drops zero-score non-survivors.** `resolveEagerRecords`
   computes the score for each candidate and, if `score === 0 &&
   !survive_compaction`, drops it before budgeting —
   [src/knowledge/loader.ts](../../../../src/knowledge/loader.ts#L246-L251).
   The synthetic record has `survive_compaction: false`, so it is
   dropped. The eager block delivered to the planner today contains
   *no* built-in skills at all — not "all four built-ins smeared
   together" as r1 implied.

Empirical confirmation: on the `saivage-v3` harness, inspecting the
planner's pre-turn eager block shows neither the `## Planning
Guidelines` heading nor any other built-in body. The planner relies
entirely on its system prompt
([prompts/planner.md](../../../../prompts/planner.md)) and calls
`plan_get` / `plan_init` / `plan_add_stage` / `run_manager` as that
prompt directs. Tokens are not wasted on a fictional body; no
incorrect plan is written.

That equilibrium is brittle in exactly one direction: **G42**.
G42 reads the YAML frontmatter on disk (including
`agentTypes: [planner]`, treated as `target_agents`), and stops
synthesising bare-topic triggers. Post-G42, the `planning` skill is
delivered to the planner role with its true file body — the dormant
fiction wakes up. The severity argument for landing G43 before G42 is
therefore "G42 is the trigger that activates the latent bug," not
"the planner currently ignores a smeared block." The sequencing rule
in [03-plan-r2.md](./03-plan-r2.md#sequencing-relative-to-g42) does
not change; only the rationale tightens.

Two downstream validation consequences (carried into
[03-plan-r2.md](./03-plan-r2.md)):

- A pre-merge sanity check is "does the planner's eager block contain
  any built-in body today?" Expected answer: no. Any other answer
  means one of the three resolver invariants above has already
  shifted, and G43's pre-condition no longer holds.
- A post-merge sanity check is "does the planner's eager block
  contain `## Planning Guidelines` or `--- SKILL: planning`?"
  Expected answer: no, both before and after the deletion. Looking
  for the *absence* of those two specific strings is safer than
  grepping for the generic word `summary` (see F7).
...
- [../G42-builtin-skills-agenttypes-silently-ignored.md](../G42-builtin-skills-agenttypes-silently-ignored.md):
  ...
  [03-plan-r2.md](./03-plan-r2.md) for the joint sequencing.
