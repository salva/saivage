...
Two proposals. B (recommended) deletes the fuzzy restart shortcut, updates the Chat system prompt to match the new contract, and adds a ChatAgent-level regression test. A is retained only as the explicitly rejected focused alternative, with the regex semantics in the analysis corrected.

## r2 deltas vs r1

1. **Prompt edit is required.** Proposal B now treats [prompts/chat.md](../../../../prompts/chat.md) as a first-class file in the change set, with the specific line edits in §B.2. The r1 "prompt nudge is optional" framing is gone.
2. **Regression test moved to ChatAgent.** Proposal B's test impact in §B.3 replaces the `dispatchLocalCommand`-only test from r1 with a ChatAgent-level test in [src/agents/agents.test.ts](../../../../src/agents/agents.test.ts), using the existing `TestChatChannel` and fake-router harness at [src/agents/agents.test.ts#L626-L655](../../../../src/agents/agents.test.ts#L626-L655). The new test must fail against the current bug.
3. **Regex semantics corrected in Proposal A.** §A.1 below no longer claims that the `u` flag makes JavaScript `\b` Unicode-aware, and no longer relies on suffix/plural matching to argue the false-positive surface. Proposal A's residual problems are restated truthfully.
4. **Transversal scope widened.** §B.4 (files touched) and §B.5 (test impact) explicitly enumerate code, prompt and test as required work, not optional notes.
...
- Noun stage: explicit alternation of `planner | planificador | planejador | planificateur | pianificatore | planer | 计划器 | プランナー | 플래너`, with the same `\p{L}\p{N}` lookaround discipline.
...
## Proposal B — Delete the regex, update the prompt, add a ChatAgent-level regression (recommended)
...
### B.2 Prompt edit (required, in-scope)

The Chat system prompt at [prompts/chat.md](../../../../prompts/chat.md) currently encodes a contract the runtime can no longer fulfill once the code shortcut is removed. The four references must be rewritten in the same change as the code deletion:

- [prompts/chat.md#L7](../../../../prompts/chat.md#L7) — the example phrase "I can restart the Planner" must be removed from the first-person examples. Chat cannot restart the Planner directly; only the `/restart-planner` slash command can. Replace with a neutral first-person example (e.g. "I have relayed that to the Planner", "I found this in the current plan") and drop the restart claim.
- [prompts/chat.md#L33](../../../../prompts/chat.md#L33) — rewrite the "You can request a Planner restart only when the user explicitly asks for it" bullet. New text: Chat cannot restart the Planner. When a user asks for a restart, answer with a single sentence directing them to type `/restart-planner <reason>` themselves. Do not claim the restart has been performed, and do not file the restart request as a note.
- [prompts/chat.md#L43](../../../../prompts/chat.md#L43) — rewrite responsibility item 5. New text: "Direct the user to the restart command on explicit request: if the user clearly asks to restart the Planner, reply with a one-line instruction to type `/restart-planner <reason>`." Drop "use the deterministic command path when available" (that path is being removed).
- [prompts/chat.md#L51](../../../../prompts/chat.md#L51) — rewrite the "Planner restart requests" bullet under "CRITICAL: Relaying User Orders". New text: "Planner restart requests (restart, reset, relaunch, abort current plan): do NOT create a note for this and do NOT claim to have restarted the Planner. Reply with the instruction to type `/restart-planner <reason>` instead. Restart is a slash-command-only action."
...
- [prompts/chat.md](../../../../prompts/chat.md) — apply the four edits enumerated in §B.2.
...
- The four prompt lines at [prompts/chat.md#L7](../../../../prompts/chat.md#L7), [prompts/chat.md#L33](../../../../prompts/chat.md#L33), [prompts/chat.md#L43](../../../../prompts/chat.md#L43), [prompts/chat.md#L51](../../../../prompts/chat.md#L51), replaced per §B.2.
...
5. Per workspace handoff, only restart `saivage.service` on `saivage-v3` (10.0.3.112) for validation; other containers are out of scope.
