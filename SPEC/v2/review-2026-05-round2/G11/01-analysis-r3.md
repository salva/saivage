...
The reviewer's three required changes ([./04-review-r2.md](./04-review-r2.md)) are addressed here:

1. *Fifth prompt directive added to the bug surface.* §6 now lists [prompts/chat.md#L73](../../../../prompts/chat.md#L73) — "Restart cautiously: Only restart the Planner when the user explicitly asks to restart it." — alongside the four r2 references. The "Guidelines" bullet at L73 also encodes a contract Chat can no longer honour once the fast path is gone, so the r3 design and plan remove it in the same change. The negative grep in the plan is widened in [./03-plan-r3.md](./03-plan-r3.md) to catch stale phrases including "Restart cautiously" and "Only restart the Planner".

2. *Regex-semantics errors fixed.* §2 below acknowledges that the live code at [src/agents/chat.ts#L353-L354](../../../../src/agents/chat.ts#L353-L354) uses the `i` flag on both regexes (r2 wrongly said neither flag was used). §2 also corrects the `\bplanner\b` claim: in JavaScript the apostrophe is not a word character, so `/\bplanner\b/i` matches the `planner` prefix in `planner's` (`r` is a word char, `'` is not — there is a boundary between them). It still does not match `planners` because both `r` and `s` are word chars and no boundary sits between them. The false-positive table in §3 is widened with a `planner's` row to reflect this.

3. *Cross-finding grep replaced with a real one.* The r2 cross-finding grep `git grep -nE "\b/(\b\(restart\|reset\|relaunch\)\b|/i" src/` would not have matched the live regex at all; it was shaped around a slash-prefixed word boundary that does not appear in [src/agents/chat.ts#L352-L355](../../../../src/agents/chat.ts#L352-L355). The new plan replaces it with literal searches for `tryHandleExplicitPlannerRestart`, the `restartPlanner(this.localCommandContext(` call shape, and the regex source text `\\b(restart|reset|relaunch)\\b`. Each fails on `main` (returns matches) and passes only after step 1–3 of the plan land (returns no matches).

The four r2 changes (prompt edit required; test moved to ChatAgent; regex semantics; transversal scope) are kept and carried forward unchanged.
...
The live regexes at [src/agents/chat.ts#L353-L354](../../../../src/agents/chat.ts#L353-L354) use the `i` flag and no `u` flag. JavaScript word boundaries are ASCII by default: `\b` is the boundary between `[A-Za-z0-9_]` and any other character (or between a word char and string edge). With `i` and no `u`:

- `/\b(restart|reset|relaunch)\b/i` matches only the three exact tokens (case-insensitive). It does not match `restarting`, `restarts`, `restarted`, `relaunched`, `resetting`, `relaunching`, etc., because the trailing letter sits on the inside of the word and there is no `\b` between two word characters.

- `/\bplanner\b/i` matches the exact token `planner` (case-insensitive). The relevant edge cases:
  - `planners` — does **not** match. Both `r` and `s` are word characters, so no `\b` sits between them; the trailing `\b` cannot succeed at that position.
  - `planner's` — **does** match the `planner` prefix. The apostrophe `'` is **not** a JavaScript word character (the `\w` class is `[A-Za-z0-9_]`), so there is a `\b` between `r` and `'`. `/\bplanner\b/i.test("planner's")` returns `true`. (r2 stated the opposite; that was wrong.)
  - `re-planner`, `super-planner` — `-` is not a word char either, so `\b` precedes `planner`; the regex matches.
  - `planner.` / `planner,` / `planner!` — punctuation is not a word char; matches.
  - `planner_v2` — `_` **is** a word char; no boundary between `r` and `_`; does **not** match.

- Adding the `u` flag does not turn `\b` into a Unicode-aware boundary. In JavaScript (unlike some other regex engines), `\b` remains ASCII-only even with `u`; `u` only affects Unicode escapes, surrogate pair handling, and a few class semantics. Any "multilingual" extension to this regex must rely on explicit alternations and `\p{L}\p{N}` lookarounds, not on `\b` doing Unicode work.
...
| `What was the planner's reason for the last restart?` | `restart` + `planner` (boundary between `r` and `'` is a JS word boundary, so `\bplanner\b` matches the prefix in `planner's`) | A question about history |
...
The Chat system prompt at [prompts/chat.md](../../../../prompts/chat.md) currently tells the LLM, in five distinct places, that Chat can restart the Planner directly and that natural-language restart requests are an interrupt path:
...
- [prompts/chat.md#L73](../../../../prompts/chat.md#L73) — Guidelines: "Restart cautiously: Only restart the Planner when the user explicitly asks to restart it. Explain that the new Planner reloads plan/history from disk …".

All five say or imply that Chat itself is a restart actor. Today they are partly implemented by the regex fast path: when a free-text restart request arrives, the code intercepts it before the LLM ever sees it, so the prompt's "request a Planner restart" and "restart cautiously" sentences are effectively a no-op for the model. After deleting the code shortcut, the same free-text request flows into the LLM turn at [src/agents/chat.ts#L207-L213](../../../../src/agents/chat.ts#L207-L213). If any of these five instructions is left as-is, the LLM is free to claim it restarted the Planner (it cannot — there is no `restart_planner` tool exposed to it) or to file the restart as a note (the wrong control surface). Either is a regression.

All five must be rewritten in the same change as the code deletion so the LLM contract matches the new runtime behaviour: natural-language restart requests are answered by directing the user to `/restart-planner <reason>`, and Chat must never claim to have restarted the Planner. The L73 "Restart cautiously" guideline is removed outright — there is no remaining Chat-side restart action to be cautious about, and a positive instruction (direct the user to the slash command) is already present at L43 and L51.
...
- **Prompt**: rewrite the five lines in [prompts/chat.md](../../../../prompts/chat.md) listed in §6 so the LLM contract matches the new behaviour (direct the user to `/restart-planner`, never claim to have restarted, never file restart-as-a-note, drop the "restart cautiously" guideline).
