...
**Design**: [02-design-r1.md](02-design-r1.md) (Proposal C — delete)
...
At line 11–19 the import group reads:
...
2. Focused vitest (the only file that exercises the deleted surface):
...
3. Full vitest:
   ...
   Expected: green. No other test imports `appendDoc` (verified by `grep -R "appendDoc" src/ tests/` → only the three matches in `src/store/documents.test.ts` removed by this plan).
...
5. Optional sanity (only if shipping a doc update in the same PR — not required to land the code change):

   ```bash
   npm run docs:api
   ```

   Expected: typedoc regenerates the `docs/api/store/documents/` tree without an `appendDoc` entry; the sidebar JSON and hashmap are rewritten accordingly. No hand-edit to `docs/api/` is part of this plan.
...
- `grep -R "appendDoc" src/ tests/` returns zero matches.
- `npx tsc -p tsconfig.json --noEmit` clean.
- `npm test` green.
- `npm run build` clean.
- No new files. No new exports. No new locks.
