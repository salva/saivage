...
Prereq: [01-analysis-r1.md](./01-analysis-r1.md).
...
1. Replace the `interface SaivageRuntime { … }` block at [docs/internals/server.md](../../../../docs/internals/server.md#L26-L36) with the verbatim text of the interface in [src/server/bootstrap.ts](../../../../src/server/bootstrap.ts#L48-L67) (13 fields including comments). Add a sentence explicitly noting the comment annotations (e.g. "Background log-only supervisor for stuck-agent detection").
2. Replace the `startServer` signature at [docs/internals/server.md](../../../../docs/internals/server.md#L42-L50) with the real one (returns `{ close(): Promise<void> }`; `ServerOptions` required, not optional in JS even though the parameter has a default). Add a one-liner noting that the only methods on the returned object are `close`.
3. Rewrite the "Graceful shutdown" section ([docs/internals/server.md](../../../../docs/internals/server.md#L70-L80)) into two subsections:
   - "CLI-driven teardown (`serve` command)" — five steps as actually performed at [src/server/cli.ts](../../../../src/server/cli.ts#L353-L386), explicitly naming `serve` as the owner.
   - "`runtime.shutdown()`" — the seven steps from [src/server/bootstrap.ts](../../../../src/server/bootstrap.ts#L218-L237), with a note that `start` and `inspect` use this directly and never touch Fastify or Telegram.
4. Drop the `spawn: ChildSpawner` line entirely; mention in prose that the child-spawner is constructed by the free function `createChildSpawner(runtime)` at [src/server/bootstrap.ts](../../../../src/server/bootstrap.ts#L256-L262) and is not stored on the runtime object.
...
1. Add a tiny VitePress markdown loader (or a `tsup`-time markdown preprocessor — same effect) that recognises a directive of the form
   ...
   and, at docs-build time, replaces the line with a fenced `ts` code block extracted from the TS file. Implementation: parse the named file with the project’s existing TypeScript compiler API (or just regex-locate `export interface <symbol>` … matching brace) and emit the bracketed body. No persistent generated file on disk; the snippet lives only in the built site.

2. Use the directive for:
   - `SaivageRuntime` in `docs/internals/server.md`.
   - `ServerOptions` and the `startServer` *declaration line* in `docs/internals/server.md`.
   - The same directive can be applied to G40 / G44 to close the recurrence in one move (out of scope for the implementation plan here, but it makes the cost-benefit work).
...
- The TS-parsing helper must handle the realistic shapes used in `bootstrap.ts` (interface with JSDoc comments, exported types) — small but not zero risk.
...
- G45 is a documentation-drift finding. The smallest correct doc rewrite is the most reviewer-vetted and lowest-risk fix and closes the actual harm — readers see truthful prose tomorrow.
...
- "Bootstrap" prose stays. Step 3 of the bullet list keeps `EventBus`, `ModelRouter`, `McpRuntime`, `NoteManager`, `Recovery`, `Supervisor`; add `PlanService`, `PlannerControl`, `RuntimeTracker` — they are constructed in `bootstrap` too. Drop the `ChildSpawner` bullet (it is not constructed in bootstrap; it is built on demand by `createChildSpawner`).
- The TS interface block is replaced by the 13-field interface above, in source order, with the JSDoc comments preserved.
- The `startServer` block is replaced by the real signature returning `{ close(): Promise<void> }`.
- A new short paragraph names the two CLI entry points that long-run vs one-shot: `serve` (long-running, calls `startServer` + `runPlannerWithRecovery`) and `start` (one-shot, calls `runPlanner`). `runPlanner` lives in [src/server/bootstrap.ts](../../../../src/server/bootstrap.ts); the doc currently conflates the two.
...
- No reference to `runtime.bus`, `runtime.mcp`, `runtime.spawn`, `runtime.abort`, `{ stop(): Promise<void> }`, or `status: "stopped"` survives anywhere in the rewritten file.
