...
Reviewer-required corrections traced into this round (see
...
restated below for self-containment so r2 is the canonical analysis.
...
([02-design-r2.md](02-design-r2.md)): **delete the cache instead
...
4. **`resolveProjectRoot` stays sync.** Bounded parent walk invoked
   ...
   `existsSync` allow-list is **narrowed to `src/config.ts`** so
   the carve-out cannot leak to unrelated modules (see
   [02-design-r2.md](02-design-r2.md) §"Regression guard").
