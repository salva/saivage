...
[04-review-r1.md](04-review-r1.md#L13-L42) applied below:
...
- **Step 6 fixtures**: new test cases reuse the existing sync
...
- **Step 7 regression test**: tightened to one shape — scan `src`
  with the **default** allow-list, then post-filter to
  `src/config.ts`, asserting exactly one violation (`existsSync`
  sync-call). `existsSync` is **not** broadened workspace-wide.
...
[01-analysis-r2.md](01-analysis-r2.md) §2.
...
## Step 6 — Add the no-sync-fs regression test
...
describe("src/config.ts is async-fs only", () => {
  it("permits only existsSync in resolveProjectRoot; nothing else sync", async () => {
    const all = await scanForSyncFs({
      roots: ["src"],
      // Default allow-list ["createWriteStream"] (G30).
      // existsSync is NOT broadened workspace-wide; it is narrowed
      // to src/config.ts via the post-filter below.
    });
    ...
    expect(configViolations[0]).toEqual({
      file: configViolations[0].file,
      kind: "sync-call",
      detail: "existsSync",
    });
  });
...
Why this exact shape (reviewer-required, finding #2):

- `roots: ["src"]` matches the shipped G30 scanner API
...
- If a second `existsSync` call (or any other `*Sync`) appears in
  `src/config.ts`, `configViolations.length` exceeds 1 and the
  assertion fails.
...
4. Final `node:fs` named-import surface in `src/` after the three
   ...
   in `src/config.ts` `resolveProjectRoot` (allow-listed by G37
   via the post-filter), plus documented out-of-scope carve-outs
   in `bootstrap` (fatal `writeFileSync`) and `runtime/stash.ts`.
...
[02-design-r2.md](02-design-r2.md) §Risk #5.
