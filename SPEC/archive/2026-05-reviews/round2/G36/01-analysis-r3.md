...
**Round references**: [01-analysis-r2.md](01-analysis-r2.md), [04-review-r2.md](04-review-r2.md)
...
Reviewer r2 ([04-review-r2.md](04-review-r2.md)) accepted every r1
fix and flagged one execution-path issue: the proposed
child-process concurrency tests use
`dist/auth/__fixtures__/concurrent-writer.js`, but the build
([tsup.config.ts](../../../../tsup.config.ts#L4-L9)) has a single
entry — `src/server/cli.ts` — and emits only `dist/cli.js`. The
fixture path does not exist, and the same mismatch applies to the
manual scratch-container command that imports
`/opt/saivage/dist/auth/index.js`.

R3 analysis is identical to r2 below except for §8 (new): the
concurrency-test runtime relies on `tsx` (already a dev dep, see
[package.json](../../../../package.json#L47)) rather than on the
bundled `dist/` tree, and the manual probe imports the in-process
store helpers from a `tsx`-executed scratch script rather than from
a non-existent `dist/auth/` module. All other call-site
enumerations, lock-protocol facts, and propagation surfaces are
unchanged.
...
- [src/server/cli.ts](../../../../src/server/cli.ts#L538) — bare
  ...
  of `auth-profiles.json` outside `store.ts`.
...
3. **Concurrent-writer races are not theoretical.** Three writers can
   ...
   From r2 on there is **no cache**: every locked mutation reloads, so
   the only way to lose a write is for two writers to both hold the
   lock at once, which the lock itself rules out for cooperating
   processes.
...
- Wait: exponential backoff (10 ms × 1.5, cap 1 s, total ≤ 10 s) on
  `EEXIST`.
- Stale recovery: before each retry, read the existing lockfile JSON.
  If `hostname` matches the current host and `process.kill(pid, 0)`
  throws `ESRCH`, the owning process is gone; `unlink(lockPath)` and
  retry immediately. If `hostname` differs we cannot prove staleness
  across hosts and continue to back off until the timeout.
...
## 8. Test-runtime constraint (r3 addition)

The cross-process tests in §3.4 / design r2 cases 2 & 3 fork a child
that calls the production `saveProfile`. Two facts determine the
fork target:

- `tsup` is configured with a single entry,
  [tsup.config.ts](../../../../tsup.config.ts#L4-L9), and emits only
  `dist/cli.js` (plus the `dist/skills/` and `dist/prompts/`
  side-effects). No `dist/auth/__fixtures__/concurrent-writer.js`
  artefact exists, and no `dist/auth/index.js` artefact exists. The
  reviewer caught that r2's plan referenced both paths as if they
  did.
- `tsx` is already a dev dep
  ([package.json](../../../../package.json#L47)), used today by
  vitest's TypeScript loading. Forking the fixture through `tsx`
  (`fork(fixturePath, [], { execArgv: ["--import", "tsx"] })`) gives
  the child direct access to the `.ts` source tree, no build step,
  and inherits the parent's `tsconfig` resolution. This is the
  smallest-blast option of the three the reviewer enumerated.

R3 picks **the tsx-fork option**. No new tsup entries. No per-test
`tsc` build step. The fixture stays as a single `.ts` file under
`src/auth/__fixtures__/` and the test invokes it via `child_process.fork`
with `execArgv: ["--import", "tsx"]`. The manual scratch-container
probe (validation §) drops the `dist/auth/index.js` import path and
runs `tsx` against an inline scratch script that imports the
`src/auth/index.ts` source from the bind-mounted host checkout —
both v2 harness containers already mount the host's
`/home/salva/g/ml/saivage` tree, so the source is reachable inside
