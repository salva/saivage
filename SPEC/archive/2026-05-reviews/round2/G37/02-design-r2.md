...
**Round-1 review**: [04-review-r1.md](04-review-r1.md) — CHANGES_REQUESTED.
...
## Round-2 deltas (reviewer-driven)

Mapping reviewer-required changes
([04-review-r1.md](04-review-r1.md#L13-L42)) to this round:
...
2. **Medium finding #2** (regression guard scope): the new
   [src/config.no-sync-fs.test.ts](../../../src/config.no-sync-fs.test.ts)
   uses the shipped G30 scanner API (`roots`/`extensions`/
   `skipPathContains`/`allowedNamedImports`) with `roots: ["src"]`
   and the **default** allow-list (`["createWriteStream"]`), then
   applies an **explicit post-filter** to violations whose `file`
   ends with `src/config.ts`. The remaining set must equal exactly
   one item: `{ kind: "sync-call", detail: "existsSync" }` from
   [src/config.ts](../../../src/config.ts#L208) in
   `resolveProjectRoot`. This keeps the `existsSync` carve-out
   scoped to `config.ts`; sync-fs in any other module is *not*
   silenced by this test (the relevant module owns its own
   regression test under G30/G36).
...
and survives r2 unchanged in shape. Proposal B (reactive
`ConfigStore` with `fs.watch` + listeners) is rejected for the same
reasons given in r1 §"Why this is rejected" (analysis §3 enumerates
every downstream snapshot consumer that would have to become
reactive — out of scope).
...
hazard. Every call re-parses `saivage.json`; analysis §3 shows the
realistic call rate is "once per bootstrap plus a few per OAuth
refresh", which makes the parse cost trivial.
...
[01-analysis-r2.md](01-analysis-r2.md) §7 risk 4 — it is a
...
out to `config.ts` only (see "Regression guard" below).
...
`roots: ["src"]` and uses an **explicit post-filter** to isolate
...
describe("src/config.ts is async-fs only", () => {
  it("permits only existsSync in resolveProjectRoot; nothing else sync", async () => {
    const all = await scanForSyncFs({
      roots: ["src"],
      // Default allow-list: createWriteStream only (G30).
      // We do NOT broaden existsSync here; we narrow it to config.ts
      // via the post-filter below.
    });
    const configViolations = all.filter((v) =>
      v.file === `src${sep}config.ts` || v.file.endsWith(`${sep}src${sep}config.ts`),
    );
    // The only permitted sync use in this module is the existsSync
    // call inside resolveProjectRoot (L208). Anything else means a
    // regression in src/config.ts itself.
    expect(configViolations).toEqual([
      { file: configViolations[0]?.file ?? "src/config.ts", kind: "sync-call", detail: "existsSync" },
    ]);
  });
});
```
...
- Any new sync surface inside `src/config.ts` (a second
  `existsSync` call, a `readFileSync`, etc.) makes `configViolations.length`
  exceed 1 and fails the test.
...
### Risk (unchanged from r1 except #5 newly explicit)
...
4. `resolveProjectRoot` stays sync (justified above).
5. **G36-rebase coupling.** If G36 is reverted, G37 must be
   ...
   ([03-plan-r2.md](03-plan-r2.md)) gates Step 1.d (`ensureDir`
