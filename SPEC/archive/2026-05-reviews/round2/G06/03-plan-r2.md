# G06 — Implementation Plan (r2)

**Finding**: [../G06-stash-uses-sync-fs.md](../G06-stash-uses-sync-fs.md)
**Analysis**: [01-analysis-r2.md](./01-analysis-r2.md)
**Design**: [02-design-r2.md](./02-design-r2.md) — Proposal A (in-place async migration; defer the `LockedJsonFile<T>` extraction).
...
- Step 3 (base-agent migration) now trims the unused `readStash, cleanStash` names from [src/agents/base.ts#L36](../../../../src/agents/base.ts#L36) so the file imports only `stashResult`. Required by [eslint.config.js](../../../../eslint.config.js#L8-L15) `no-unused-vars: "error"`.
- Step 9 (round-trip test) rewritten: import `beforeEach` / `afterEach` from `vitest` (not from `vi`); preserve and restore the two env vars [src/config.ts#L199-L221](../../../../src/config.ts#L199-L221) actually reads (`PROJECT_ROOT`, `SAIVAGE_ROOT`); explicitly `mkdirSync(<tempDir>/.saivage)` before the first stash call; assert that returned paths live under `<tempDir>/.saivage/tmp/stash/`. The pattern mirrors [src/mcp/builtins.test.ts](../../../../src/mcp/builtins.test.ts#L37-L67).
- Step 7 (caller cascade verification) tightened to expect a trimmed import line on `src/agents/base.ts`.
...
2. **Rewrite [src/runtime/stash.ts](../../../../src/runtime/stash.ts) end-to-end.** Replace the file's contents with the final shape in [02-design-r2.md §"Final shape of `src/runtime/stash.ts`"](./02-design-r2.md). Concretely:
   ...
   - Change `export function cleanStash(maxAgeMs = ...)` ([L57](../../../../src/runtime/stash.ts#L57)) to `export async function cleanStash(maxAgeMs = ...): Promise<number>`. Replace the sync `for` loop ([L64-L72](../../../../src/runtime/stash.ts#L62-L72)) with the `Promise.all`-over-`readdir`-entries shape from [02-design-r2.md](./02-design-r2.md). The `try { ... } catch { /* ignore */ }` around `stat`/`unlink` is preserved verbatim around the new `await stat` / `await unlink` calls. Sum the per-entry `1|0` returns; preserve the `log.info` and the `return removed;`.

3. **Migrate `src/agents/base.ts` — trim the stash import and convert `maybeStash`** (r2).
   - At [src/agents/base.ts#L36](../../../../src/agents/base.ts#L36), replace `import { stashResult, readStash, cleanStash } from "../runtime/stash.js";` with `import { stashResult } from "../runtime/stash.js";`. `readStash` and `cleanStash` have zero in-file references (verified by `grep -n "readStash\|cleanStash" src/agents/base.ts` — only the L36 import matches; without this trim, the lint gate in step 10 fails with `@typescript-eslint/no-unused-vars`).
   - At [src/agents/base.ts#L697](../../../../src/agents/base.ts#L697) change the signature from `private maybeStash(content: string, toolUseId: string): string` to `private async maybeStash(content: string, toolUseId: string): Promise<string>`.
   - At [L704](../../../../src/agents/base.ts#L704) change `const path = stashResult(...)` to `const path = await stashResult(...)`. The `tokenBudget` / `countTokens` / early-`return content` block is untouched.
...
5. **Migrate `src/runtime/dispatcher.ts` — `read_stash` synthetic tool.** At [src/runtime/dispatcher.ts#L150](../../../../src/runtime/dispatcher.ts#L150) change `const result = readStash(args.path, args.offset ?? 0, args.length ?? 10_000);` to `const result = await readStash(args.path, args.offset ?? 0, args.length ?? 10_000);`. The enclosing `private async executeLocalTool(...)` ([L139](../../../../src/runtime/dispatcher.ts#L139)) is already `async`. No other dispatcher edit. Import at [L13](../../../../src/runtime/dispatcher.ts#L13) is a single name (`readStash`) and remains unchanged.

6. **Migrate `src/server/bootstrap.ts` — startup `cleanStash`.** At [src/server/bootstrap.ts#L204](../../../../src/server/bootstrap.ts#L204) change `cleanStash();` to `await cleanStash();`. The enclosing `export async function bootstrap(...)` ([L110](../../../../src/server/bootstrap.ts#L110)) is already `async`. No other bootstrap edit. The `// 9. Clean stale stash files` comment is untouched. Import at [L14](../../../../src/server/bootstrap.ts#L14) is a single name (`cleanStash`) and remains unchanged.

7. **Confirm no other callers exist and the import trim is the only base-agent import edit.** Run:

   ```bash
   grep -rn "stashResult\|readStash\|cleanStash" src/ web/ --include='*.ts' --include='*.vue'
   ```

   Expected matches:
   - the new `src/runtime/stash.ts` definitions;
   - the three caller edits in steps 3-6;
   - the trimmed import `import { stashResult } from "../runtime/stash.js";` at [src/agents/base.ts#L36](../../../../src/agents/base.ts#L36) — note no `readStash`/`cleanStash` on this line after step 3;
   - the unrelated local-symbol `const readStash: Formatter` at [web/src/utils/toolFormatters.ts#L579](../../../../web/src/utils/toolFormatters.ts#L579) (not an import from the runtime stash module).

   If any other call site exists (e.g. introduced concurrently by another branch), surface it before continuing.
...
8. **Add per-subsystem regression guard.** New file `src/runtime/no-sync-fs.test.ts`:
   ...
   Do **not** re-implement the scanner. Do **not** copy it into `src/runtime/`. Import from `../testing/noSyncFsScanner.js` (the file produced by G30 — see [../G30/02-design-r2.md](../G30/02-design-r2.md)). The `recovery.ts` carve-out follows the rationale in [01-analysis-r2.md §5](./01-analysis-r2.md) and the round-1 F22 notes — `recovery.ts` intentionally uses sync fs in exit-path handlers.

9. **Add round-trip test** (r2 fixture). New file `src/runtime/stash.test.ts` containing the four cases from [02-design-r2.md §"Round-trip test (r2 fixture)"](./02-design-r2.md) verbatim. Key fixture requirements (all enforced by the test code itself):
   - Import `describe, it, expect, beforeEach, afterEach` from `"vitest"`. **Do not** use `vi.beforeEach` / `vi.afterEach` — those are not Vitest hooks and would not type-check.
   - In `beforeEach`: `projectRoot = mkdtempSync(join(tmpdir(), "saivage-stash-"))`; capture `previousProjectRoot = process.env["PROJECT_ROOT"]` and `previousSaivageRoot = process.env["SAIVAGE_ROOT"]`; set `process.env["PROJECT_ROOT"] = projectRoot` and `process.env["SAIVAGE_ROOT"] = join(projectRoot, ".saivage")`; `mkdirSync(join(projectRoot, ".saivage"), { recursive: true })`; compute `stashRoot = join(projectRoot, ".saivage", "tmp", "stash")` for later containment assertions.
   - In `afterEach`: restore both env vars (delete if previously undefined; otherwise reassign); `rmSync(projectRoot, { recursive: true, force: true })`.
   - Each `stashResult` call site in the test asserts `path.startsWith(stashRoot)` so that any future fixture regression (env-var capture broken, `saivageDir()` ignoring the env) fails loudly instead of silently writing to the repo's `.saivage/`.
   - For the `cleanStash` case import `utimes` from `node:fs/promises` and backdate the two files to `Date.now() - 10 * 60_000`, then call `await cleanStash(60_000)` and assert `removed === 2`. Verify the surviving file is still readable via `readStash`.
   - Use `process.env["KEY"]` indexed access (matches the repo's `noUncheckedIndexedAccess` style).
...
10. **Type-check, lint, and build.** Run, in order:
    ...
    - `npm run lint` — expect zero new errors. The trimmed import on `src/agents/base.ts` (step 3) is what makes this gate pass; without it, `@typescript-eslint/no-unused-vars` flags `readStash` and `cleanStash` as errors (rule is `"error"`, not `"warn"`, per [eslint.config.js](../../../../eslint.config.js#L8-L15)).
...
### Static — caller cascade and import trim (r2)

```bash
cd /home/salva/g/ml/saivage
grep -n "stashResult\|readStash\|cleanStash" src/agents/base.ts src/runtime/dispatcher.ts src/server/bootstrap.ts
# Expected:
#   src/agents/base.ts:36:   import { stashResult } from "../runtime/stash.js";   (trimmed — no readStash, no cleanStash)
#   src/agents/base.ts:339:  content: await this.maybeStash(r.content, r.toolUseId),
#   src/agents/base.ts:704:  const path = await stashResult(...);                  (inside async maybeStash)
#   src/runtime/dispatcher.ts:13:    import { readStash } from "./stash.js";
#   src/runtime/dispatcher.ts:150:   const result = await readStash(...);
#   src/server/bootstrap.ts:14:      import { cleanStash } from "../runtime/stash.js";
#   src/server/bootstrap.ts:204:     await cleanStash();

# Explicit no-unused-imports check on the trimmed line
grep -n "^import .* from \"\\.\\./runtime/stash\\.js\";" src/agents/base.ts
# Expected exactly one line, equal to:
...
Single-commit revert is the rollback. The change is internal to a single source file, one trimmed import, three caller awaits, and two new test files; no on-disk format change, no schema migration, no new dependencies.

```bash
cd /home/salva/g/ml/saivage
git revert <commit-sha>           # restores pre-G06 sync stash.ts, the three caller sites, and the original base.ts import line
npm run build
sudo lxc-attach -n saivage-v3 -- systemctl restart saivage.service
curl -fsS http://10.0.3.112:8080/health
```
...
The production change (the async migration and the import trim) stays in place.
...
- **G36 ([../G36/APPROVED.md](../G36/APPROVED.md))** — chose in-place for `src/auth/store.ts` with a locked RMW helper. G06 reuses the precedent (in-place) and the rationale for *not* extracting a `LockedJsonFile<T>` primitive here (see [02-design-r2.md §"Proposal B"](./02-design-r2.md) and G36's own deferred note at [../G36/02-design-r1.md L322-L332](../G36/02-design-r1.md)). **No code-level coordination with G36 required** — disjoint modules.
...
Per [01-analysis-r2.md §8](./01-analysis-r2.md):
