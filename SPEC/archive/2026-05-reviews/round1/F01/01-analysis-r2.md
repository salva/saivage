...
- **Accepted (reviewer required change #1):** added [src/agents/manager.ts L260](../../../../src/agents/manager.ts#L260) — the Manager prompt's "Valid `type` / `assigned_to`" sentence — as an additional role-enumerating surface. r1 missed it; verified against the live file (the sentence currently reads `Valid \`type\` values are \`code\`, \`research\`, \`data\`, \`review\`, \`test\`, \`document\`. Valid \`assigned_to\` values are \`coder\`, \`researcher\`, \`data_agent\`, \`reviewer\`.`).
- **Updated the contract table** below to list this surface, so design/plan r2 can reference it.
- Everything else in r1 stands; the orphan evidence, F09 entanglement, operator constraint, and constraints section are unchanged.
