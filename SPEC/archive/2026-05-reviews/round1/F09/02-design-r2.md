# F09 — Design proposals (R2)

## Changes from r1

Reviewer feedback in [04-review-r1.md](04-review-r1.md):

- **Reviewer item 1 (stale refs)**: accepted. Line refs in this file re-verified against the current source. The behavioural references in Proposal C now point to `reviewer.ts` L102-L104 (the `run()` delegate) and L106-L121 (the `review()` body), and to `base.ts` L269 for the terminal assistant push.
- **Reviewer item 2 (reviewer-run semantics)**: accepted. Proposal C now states explicitly that `ReviewerAgent` keeps its own `run()` override that is a one-line delegate to `this.review(this.input)`, and that `review()` is the only method that touches `reviewCount` / `injectMessage`. `ReviewerAgent` does **not** inherit `WorkerAgent.run()`. The `run()` body that lives on `WorkerAgent` is used by `CoderAgent` / `ResearcherAgent` / `DataAgent` only.
- **Reviewer item 3 (validation skill)**: no design-level change. Handled in [03-plan-r2.md](03-plan-r2.md).
...
- [coder.ts](../../../../src/agents/coder.ts): module-private `normalizeTask` ([L212](../../../../src/agents/coder.ts#L212)), `parseTaskReport` ([L263](../../../../src/agents/coder.ts#L263)), `buildFailureReport` ([L319](../../../../src/agents/coder.ts#L319)) deleted (lines 212-end of those bodies, ~140 lines total). Top adds `import { normalizeTask, parseTaskReport, buildFailureReport } from "./task-report.js";`. All three callsites pass `"coder"` as the role. `CODER_PROMPT`, `buildCoderMessage`, `CoderAgent` class, and the unused `readFileSync`/`join` imports stay or are cleaned. Net: ~140 lines removed.
- [researcher.ts](../../../../src/agents/researcher.ts): same. Helpers at [L208](../../../../src/agents/researcher.ts#L208), [L260](../../../../src/agents/researcher.ts#L260), [L313](../../../../src/agents/researcher.ts#L313). Net: ~140 lines removed.
- [data-agent.ts](../../../../src/agents/data-agent.ts): same. Helpers at [L125](../../../../src/agents/data-agent.ts#L125), [L176](../../../../src/agents/data-agent.ts#L176), [L229](../../../../src/agents/data-agent.ts#L229). Net: ~140 lines removed.
- [reviewer.ts](../../../../src/agents/reviewer.ts): same, plus `normalizeWorkerInput` at [L143](../../../../src/agents/reviewer.ts#L143) becomes `{ ...input, task: normalizeTask(input.task, "reviewer") }` inline at its single use site inside `review()`. The stray `this.messages.push({ role: "assistant", content: text })` at [L121](../../../../src/agents/reviewer.ts#L121) is **not** in F09's scope under Proposal A and stays. Net: ~145 lines removed.
...
- 1 file deleted (`designer.ts`, ~267 lines).
...
- The `run()` skeleton remains duplicated in 4 worker files. The body of `run()` is not touched.
...
export abstract class WorkerAgent extends BaseAgent implements Agent {
  ...
  /**
   * Shared try/catch/finishReason → AgentResult mapping. Used by run() on the
   * three pure workers (coder, researcher, data-agent) and by ReviewerAgent's
   * own review() method (which adds its own pre-loop normalisation and message
   * injection before calling this).
   */
  protected async executeTask(input: WorkerInput): Promise<AgentResult> {
    ...
    try {
      ...
      if (finishReason === "abort" || finishReason === "cancelled") {
        return { kind: "abort", reason: text, partial: buildFailureReport(input, this.workerRole, startedAt, startMs, text) };
      }
      if (finishReason === "max_compactions" || finishReason === "error") {
        return { kind: "failure", reason: text, partial: buildFailureReport(input, this.workerRole, startedAt, startMs, text) };
      }
      return { kind: "success", data: parseTaskReport(text, input, this.workerRole, startedAt, startMs) };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      log.error(`[${this.workerRole}:${this.id}] Failed: ${msg}`);
      return { kind: "failure", reason: msg, partial: buildFailureReport(input, this.workerRole, startedAt, startMs, msg) };
    }
  }
...
- [coder.ts](../../../../src/agents/coder.ts): file shrinks from 343 lines to ~210 (the prompt is the bulk). Class body becomes:
...
- [reviewer.ts](../../../../src/agents/reviewer.ts): **special case — preserves the existing public contract.**

  `ReviewerAgent extends WorkerAgent` but overrides `run()` exactly the way it does today:
  ...
  - The `run()` override at [reviewer.ts L102-L104](../../../../src/agents/reviewer.ts#L102-L104) is preserved verbatim — `ReviewerAgent.run()` remains a one-line delegate to `review(this.input)`. It does **not** inherit `WorkerAgent.run()`. This is the explicit answer to reviewer item 2: the F14 / assistant-push side-effect that flows through `review(this.input)` is preserved by routing `run()` through the same `review()` body. The dispatch in [bootstrap.ts L372-L374](../../../../src/server/bootstrap.ts#L372-L374) is unchanged.
  - `reviewCount` and `injectMessage` continue to live exclusively inside `review()`.
  - `normalizeWorkerInput` at [reviewer.ts L143](../../../../src/agents/reviewer.ts#L143) is deleted; its single call site is replaced with the inline normalisation shown above.
  - The duplicate assistant push at [reviewer.ts L121](../../../../src/agents/reviewer.ts#L121) is **removed**. The terminal assistant message is already pushed by `BaseAgent.runLoop()` at [base.ts L269](../../../../src/agents/base.ts#L269); the manual push was a double-write bug (F14 / subsystem-map note). Removing it makes `review()`'s post-loop state consistent with what `executeTask()` sees for the other workers. This is the only behaviour change `ReviewerAgent` undergoes; `reviewCount`, `injectMessage`, the public `review(input)` signature, and the `run()` → `review(this.input)` delegation are all preserved exactly.
  - `parseTaskReport` and `buildFailureReport` callsites flow through `executeTask()` once for both the `run()`-driven first call and the manager-driven follow-up calls.

- [designer.ts](../../../../src/agents/designer.ts): **deleted** per F01.
- [inspector.ts](../../../../src/agents/inspector.ts): **untouched**. It uses `InspectionRequest`/`InspectionReport`, not `WorkerInput`/`TaskReport`, and is invoked through a different lifecycle (it has no manager-driven retry, no Stage scoping, no checklist). Inspector helpers are `normalizeInspectionRequest` at [inspector.ts L185](../../../../src/agents/inspector.ts#L185) and `parseInspectionReport` at [inspector.ts L219](../../../../src/agents/inspector.ts#L219); they read `InspectionRequest`/`InspectionReport` shapes that are schema-distinct from `Task`/`TaskReport`. Keeping inspector on `BaseAgent` directly preserves the right separation of concerns.
...
- Reviewer double-push bug (F14) fixed as a side-effect of the `review()` rewrite — and explicitly documented in §"After-state per worker → reviewer" above so the behaviour change is auditable.
...
- Per-worker override of the success/abort/failure mapping. None varies today; reviewer's variation is at the `review()` layer (pre-loop normalisation and message injection), not in the `executeTask()` body.

### Recommendation note

Right level of abstraction: the shared structure goes into a class that exists for exactly that reason. `BaseAgent` keeps its role as "LLM/tool/compact loop"; `WorkerAgent` keeps its role as "manager-dispatched, task-scoped, TaskReport-producing", and exposes `executeTask()` as the post-loop mapping that `ReviewerAgent.review()` reuses without giving up its own pre-loop work. Inspector stays separate because it genuinely is.
