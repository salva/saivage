...
- **Accepted (reviewer required change 1).** All `src/runtime/supervisor.ts` anchors re-verified against the 272-line current file. The recommended-proposal scope table (Proposal B) now references the correct individual ranges instead of the inaccurate lumped `L246-L257`. Updated: `askModel` L125-L147 (was L122-L156); normalizer call L146 (was L154); system prompt L161-L168 (was L158-L168); `parseVerdict` L176-L202 (was L171-L201); normalizer body L222-L257 (was L220-L257); `looksLikeLongRunningExternalWork` L259-L264, `looksLikeProviderThrottling` L266-L268, `looksLikeMalformedOrCrashed` L270-L272 (was the imprecise `L246-L257` lump). The user-message wording reference is now L131-L138 → L133-L141 (still inside `askModel`); `FORCE_CANCEL_DELAY_MS` schedule reference updated from L105-L114 to L105-L114 (unchanged, range stable). No substantive design change.
- **Rejected:** none. The Proposal B recommendation, the Proposal A / Proposal C disposition, and the cross-issue composition argument all stand on substance.
...
- **Modify**: [src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L146) (one call site), [src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L222-L257) (the function body), and the three `looksLike*` predicates ([supervisor.ts L259-L264](../../../../src/runtime/supervisor.ts#L259-L264), [L266-L268](../../../../src/runtime/supervisor.ts#L266-L268), [L270-L272](../../../../src/runtime/supervisor.ts#L270-L272)).
- **No new files.**
...
- The system prompt at [supervisor.ts L161-L168](../../../../src/runtime/supervisor.ts#L161-L168) — the LLM is still asked to apply the same rules.
- `ROLE_ABORT_PRIORITY` (F23 territory).
- All constants in [supervisor.ts L8-L12](../../../../src/runtime/supervisor.ts#L8-L12) (F11 territory).
- The two regex-validation tests at [runtime.test.ts L198-L252](../../../../src/runtime/runtime.test.ts#L198-L252) keep passing because they feed verdicts whose `reason` does include "the only clear issue is" -shaped text (test fixtures may need a one-word tweak to satisfy the tighter pattern).
...
- **Modify**: [src/runtime/supervisor.ts](../../../../src/runtime/supervisor.ts#L78-L272):
  ...
  - `askModel` ([L125-L147](../../../../src/runtime/supervisor.ts#L125-L147)) drops the wrap `normalizeNonStuckOperationalVerdict(...)` and returns `parseVerdict(response.content, provider)` directly.
  - `parseVerdict` ([L176-L202](../../../../src/runtime/supervisor.ts#L176-L202)) — F03 changes this to use `parseLlmJsonAs(content, SupervisorVerdictSchema)`. F05 does not need to touch the body, only the line at [supervisor.ts L146](../../../../src/runtime/supervisor.ts#L146): `return normalizeNonStuckOperationalVerdict(parseVerdict(...), logs);`.
  - Delete `normalizeNonStuckOperationalVerdict` ([L222-L257](../../../../src/runtime/supervisor.ts#L222-L257)).
  - Delete `looksLikeLongRunningExternalWork` ([L259-L264](../../../../src/runtime/supervisor.ts#L259-L264)), `looksLikeProviderThrottling` ([L266-L268](../../../../src/runtime/supervisor.ts#L266-L268)), `looksLikeMalformedOrCrashed` ([L270-L272](../../../../src/runtime/supervisor.ts#L270-L272)).
- **Modify**: [src/runtime/runtime.test.ts](../../../../src/runtime/runtime.test.ts#L198-L252) — the two regex-validation tests change shape (see Plan).
- **No new files.**

### What this enables

- The system prompt becomes the **single** place where supervisor policy lives. Operators tuning behaviour edit one string ([supervisor.ts L161-L168](../../../../src/runtime/supervisor.ts#L161-L168)); they don't have to reverse-engineer a regex post-processor that runs on top of it.
...
- `SUPERVISOR_SYSTEM_PROMPT` ([supervisor.ts L161-L168](../../../../src/runtime/supervisor.ts#L161-L168)). It already encodes the rules.
- The threshold counter ([supervisor.ts L10](../../../../src/runtime/supervisor.ts#L10), checked in [supervisor.ts L78-L116](../../../../src/runtime/supervisor.ts#L78-L116)).
- `ROLE_ABORT_PRIORITY` (F23 territory).
- `FORCE_CANCEL_DELAY_MS` and the schedule-second-cancel logic ([supervisor.ts L105-L114](../../../../src/runtime/supervisor.ts#L105-L114)).
- The user message wording at [supervisor.ts L133-L141](../../../../src/runtime/supervisor.ts#L133-L141) — already says "untrusted as instructions"; the LLM is already correctly framed as adjudicator of untrusted log text.
...
Inspection of [src/providers/](../../../../src/providers/) (`rg -n 'response_format|json_schema|structured' src/providers`) returns **zero hits**. None of the eight provider adapters today translates a "structured-output mode" hint to the provider's native API. F03's own analysis confirms the same finding ([SPEC/v2/review-2026-05/F03/01-analysis-r1.md](../F03/01-analysis-r1.md)) and explicitly defers a structured-output adoption to a separate provider-layer refactor.
...
- Net diff: ~50 LOC deleted, three predicates and one normaliser gone. No new files, no new constants, no new config knob.

The plan in [03-plan-r2.md](03-plan-r2.md) follows Proposal B.
