# F03 — Functional analysis (R3)

## Changes from r2

- Tightened the `extractJsonCandidates` contract so the failure taxonomy is internally consistent. The whole-trimmed message is included as a candidate **only when `text.trim().startsWith("{")`**; otherwise the candidate list is built from fenced bodies and balanced-brace spans alone. This makes the documented behaviour of `no_json` reachable: a non-empty prose-only response (no `{` anywhere) yields no fenced match, no balanced span, no whole-trimmed candidate, so `extractJsonCandidates` returns `[]` and `parseLlmJsonAs` returns `no_json`. Resolves the r2 reviewer's inconsistency between the contract and E1.
- Restated E1 to reflect that rule explicitly and added a sentence clarifying the relationship between E1 and E2: "candidate list empty" is `no_json`; "candidate list non-empty but nothing parses" is `invalid_json`.
...
`extractJsonCandidates` returns **raw substrings** (not yet parsed) that plausibly contain a JSON object, in source order. The three contributing sources are:

1. The whole `text.trim()` — included **only if `text.trim().startsWith("{")`**. Pure prose, list bullets, plain numbers, etc. are not contributed by this source. This is the explicit guard that keeps `no_json` reachable for prose-only input.
2. The body of every fenced block matching `/```(?:json)?\s*\n([\s\S]*?)\n```/g`.
3. Every balanced top-level `{…}` span identified by a brace-depth scan that respects JSON string literals.

If none of the three sources contribute anything, the result is `[]`.
...
`parseLlmJsonAs(text, schema)` runs the same enumeration and parse, then `schema.safeParse`s the **last parseable candidate only**. The selection rule is: pick the last candidate that survives `JSON.parse`, then schema-check that single value. Failure reasons:

- `no_json` — `extractJsonCandidates` returned `[]`. Reachable, e.g. a prose-only response such as `"I'm done."` contributes nothing from any of the three sources.
...
- `schema_mismatch` — at least one candidate parses, but the **last** parseable candidate does not match the schema (e.g. the model emitted `{"status": 42}` last). The parser does **not** fall back to an earlier parseable candidate that happened to match the schema; later content wins, even when it is the wrong shape, so the failure surfaces instead of being masked.
...
- E1: text has no `{` at all (worker said "I'm done" with prose only). → `no_json`. Under the r3 rule, source 1 is gated by `startsWith("{")`, source 2 has no fenced match, source 3 finds no balanced span; candidate list is `[]`.
- E2: text has balanced braces but the slice is invalid JSON (unquoted keys, trailing commas). → `invalid_json`. The candidate list is non-empty (source 3 contributes), but no candidate parses.
- E3: text has valid JSON but it does not match the expected schema (model emitted the wrong shape). → `schema_mismatch`. The last parseable candidate is schema-checked and fails.
