# F03 — Implementation plan (R2)

Plan for **Proposal B** from [02-design-r2.md](02-design-r2.md): shared `src/parse-llm-json.ts` with `extractJsonCandidates`, `parseLlmJson`, and schema-typed `parseLlmJsonAs`; silent-success branches deleted from worker / inspector / manager parsers.

## Changes from r1

- **Step 1 rewritten** to match the design's single extraction rule. The helper now exposes `extractJsonCandidates(text): string[]` returning **raw** substrings (whole-trimmed, fenced bodies, balanced-brace spans) in source order, with no parse-or-skip logic inside the extractor. `parseLlmJson` and `parseLlmJsonAs` walk that list and pick the last that parses (and, for the typed variant, that also satisfies the schema). This removes r1's contradiction where Step 1 specified fenced-first precedence while Step 2 expected the final balanced object to win.
...
`extractJsonCandidates` returns a list of **raw substrings** (no parsing inside the extractor), in source order:

1. The trimmed whole message (one candidate, even if it does not look like JSON — the parsing layer will reject it).
...
5. `const result = schema.safeParse(lastParsedValue);`
...
- `extractJsonCandidates` returns `[]` for empty input and for prose-only input with no `{` (E1).
