...
Three proposals. A is the focused reading of the ticket (promote everything). B is one conceptual level up (inline + delete; promote only what an operator actually tunes). C is a curated middle.
...
- [src/config.ts](src/config.ts) — add exactly four small additions:
...
- **Delete the `EventBus` constructor `handlerTimeoutMs` parameter**: no caller overrides it; it is currently configurable-for-no-reason. Replace with the module-scope const directly. (If a future need arises, add the parameter back.)
...
**Risk:** Low. Most changes are mechanical const renames within a file. The four promoted keys are well-bounded; defaults match today's behaviour exactly. One real risk: dropping `SAIVAGE_SHELL_TIMEOUT_FLOOR_MS` is a small operator-facing change — call it out in the plan; the test `src/mcp/builtins.test.ts` sets it and must be updated. Project policy explicitly allows breaking changes; an env-var-only knob nobody documents publicly is a clean candidate for removal.
