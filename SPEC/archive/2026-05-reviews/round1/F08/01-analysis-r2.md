...
- Corrected stale file:line references against current source:
  - `tmp/state/` runtime path declarations: [src/store/project.ts](src/store/project.ts#L74-L76) → [src/store/project.ts](src/store/project.ts#L82-L84).
  - Failure-state write in the fatal handler: [src/server/bootstrap.ts](src/server/bootstrap.ts#L688) → [src/server/bootstrap.ts](src/server/bootstrap.ts#L693-L695) (the actual `writeRuntimeState` call is at [src/server/bootstrap.ts](src/server/bootstrap.ts#L695); the surrounding `failState`/status block spans L693-L695).
  - `RuntimeTracker.flush()` hot-path writer: [src/runtime/recovery.ts](src/runtime/recovery.ts#L402) → [src/runtime/recovery.ts](src/runtime/recovery.ts#L407) (the full flush body is [src/runtime/recovery.ts](src/runtime/recovery.ts#L397-L407)).
  - Runtime-state HTTP readers in `server.ts`: replaced the single stale `L475-L498` range with the three actual reader sites — `/health` at [src/server/server.ts](src/server/server.ts#L127-L131), `/api/state` at [src/server/server.ts](src/server/server.ts#L179-L188), and `/api/debug/state` at [src/server/server.ts](src/server/server.ts#L481-L505).
  - Chat-agent status reader: [src/agents/chat.ts](src/agents/chat.ts#L347) → [src/agents/chat.ts](src/agents/chat.ts#L391-L393).
- Renamed the web UI's runtime-state endpoint from `/api/status` to its actual name `/api/state`, confirmed against [web/src/components/StatusPanel.vue](web/src/components/StatusPanel.vue#L58). No `/api/status` route exists in `server.ts`.
- No semantic change to the conclusion: the mirror is still pure dead-code duplication, and the proposed deletion contract is unchanged.
...
The only semantic asymmetry is the path: the SPEC-current path lives under `tmp/state/` (alongside `shutdown-request.json` and `shutdown-summary.json`, see [src/store/project.ts](src/store/project.ts#L82-L84)); the mirror lives directly under `.saivage/runtime/`, a directory that has no other purpose and would not exist at all if the mirror were dropped.

## Contract

`writeRuntimeState(path: string, state: RuntimeState): void`

- **Input**: the project's canonical runtime-state path (always `paths.runtimeState`) and a fully-formed `RuntimeState` (validated by `RuntimeStateSchema` in [src/types.ts](src/types.ts#L269-L276)).
...
- [src/server/bootstrap.ts](src/server/bootstrap.ts#L693-L695) — failure-state write on planner crash (the `writeRuntimeState` call itself is at [src/server/bootstrap.ts](src/server/bootstrap.ts#L695)).
- [src/runtime/recovery.ts](src/runtime/recovery.ts#L407) — `RuntimeTracker.flush()`, the hot-path writer driven by `agentStarted`/`agentStopped`/`agentActivity`/`setCurrentStage` (full flush body at [src/runtime/recovery.ts](src/runtime/recovery.ts#L397-L407)).
...
- [src/server/server.ts](src/server/server.ts#L127-L131) — `/health` endpoint surfaces `runtime.status`.
- [src/server/server.ts](src/server/server.ts#L179-L188) — `/api/state` endpoint consumed by the web UI ([web/src/components/StatusPanel.vue](web/src/components/StatusPanel.vue#L58)).
- [src/server/server.ts](src/server/server.ts#L481-L505) — `/api/debug/state` endpoint.
- [src/server/cli.ts](src/server/cli.ts#L143) — `saivage status`.
- [src/agents/chat.ts](src/agents/chat.ts#L391-L393) — chat agent's `cmdStatus` surfacing live agent state.
...
- The web UI fetches runtime state via `/api/state` ([src/server/server.ts](src/server/server.ts#L179-L188), called from [web/src/components/StatusPanel.vue](web/src/components/StatusPanel.vue#L58)), which reads `paths.runtimeState` — not the mirror.

Schemas constraining the document:

- `RuntimeStateSchema` in [src/types.ts](src/types.ts#L269-L276) — applies identically to both paths; deleting the mirror does not require any schema change.
