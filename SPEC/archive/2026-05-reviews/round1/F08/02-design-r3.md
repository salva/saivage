...
- Corrected stale file-line refs in Proposal B: `RuntimeStateSchema.status` is at [src/types.ts](src/types.ts#L269-L270) (was cited as `L287-L288`), and the bootstrap fatal-handler error-state write is at [src/server/bootstrap.ts](src/server/bootstrap.ts#L693-L695) (was cited as `L687-L689`). Verified against the current files: `src/types.ts` line 269 begins `export const RuntimeStateSchema = z.object({`, line 270 is `status: z.enum(["idle", "running", "suspended", "error"]),`; `src/server/bootstrap.ts` line 693 is `const failState = createRuntimeState();`, line 694 is `failState.status = "error";`, line 695 is `writeRuntimeState(runtime.project.paths.runtimeState, failState);`.
- No semantic changes. Recommendation remains Proposal A.
...
- [src/runtime/recovery.ts](src/runtime/recovery.ts#L297-L407) — collapse the dual-write (same as A), then:
  ...
  - Add `RuntimeTracker.setStatus(status: RuntimeState["status"]): void` so the three direct `writeRuntimeState` calls in bootstrap become tracker method calls. The accepted `status` values are exactly the existing enum `"idle" | "running" | "suspended" | "error"` from [src/types.ts](src/types.ts#L269-L270); no new status is introduced.
