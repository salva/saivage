...
- Aligned the description of `PROVIDER_TO_OAUTH` in Proposal A with the r1 plan: the constant stays **private** to [src/providers/router.ts](src/providers/router.ts) (no `export` added). It has no consumer outside `resolveApiKey` today, and adding public API surface for a hypothetical future caller would violate the "no premature configurability / no abstractions used only once" guideline. Removed the "exported for any future caller" wording. Plan r1 already specifies "No change" for `PROVIDER_TO_OAUTH`, so no plan revision is needed.
...
- [src/providers/router.ts](src/providers/router.ts) — drop the parallel `OAUTH_TO_PI` table (unused after the deletion). `PROVIDER_TO_OAUTH` remains the single mapping and remains private to this file; no `export` is added.
...
**What gets added:** nothing. `PROVIDER_TO_OAUTH` keeps its current private visibility ([src/providers/router.ts](src/providers/router.ts#L68-L73)); no new public exports are introduced.
...
- `OAUTH_TO_PI` in [router.ts#L57-L62](src/providers/router.ts#L57-L62) — confirm no callers; if any consumer needs the OAuth→pi-ai mapping after the change, it can derive it from `PROVIDER_TO_OAUTH` inverted (still private).
...
**What it forbids:** any future code that resolves a key without going through `resolveApiKey`. (Today three call sites already follow that pattern — `router.chat`, `router.listModels`, `inspectUsageCandidate` plus the security pre-flight — deleting the bootstrap injector makes it a convention.)
...
**Recommendation note:** this is the minimal correct fix and matches the architecture-first guideline (delete the dead path, do not preserve it). Keeping `PROVIDER_TO_OAUTH` private avoids adding API surface that no caller needs.
...
- [src/providers/router.ts](src/providers/router.ts) — change `chat`, `listModels`, `inspectUsageCandidate` to thread the resolved key through the request (e.g. add an internal `_apiKey` field on the request object passed to `provider.chat`/`provider.listModels`, or pass an explicit `apiKey` argument). The provider implementations consume the per-call key instead of storing it. `callProvider` itself does not need to change: it forwards the request `router.chat` already prepared.
...
- Every call to `provider.setApiKey(...)` in [router.ts](src/providers/router.ts) (`router.chat` at L297-L299, `router.listModels` at L236-L238, `inspectUsageCandidate` at L638-L640) and in [prompt-injection-cop.ts](src/security/prompt-injection-cop.ts#L113).
...
The architecture-first guideline applies cleanly: there is no migration step, no flag, no deprecation. The eager path is removed in the same commit as the (already canonical) lazy path becomes the only path. `PROVIDER_TO_OAUTH` stays private; promoting it to a public export would add unused API surface.
