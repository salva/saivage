...
saivage/src/rag/
  ...
  ingest/
    ...
    lock.ts                 // per-dataset file lock (proper-lockfile-equivalent, no new dep)
  cache/
...
export interface QueryOptions {
  ...
  filter?: QueryFilter;
  overshoot?: number;     // multiplier for post-filter; default 3
}
...
operator / future consumer
  ...
  -> acquire per-dataset file lock under <projectRoot>/.saivage/rag/<datasetId>/ingest.lock
  -> walker.walk(input)
...
The store-adapter decides pre-filter vs. post-filter internally. For sqlite-vec, equality and `in` predicates on indexed metadata columns become a candidate-rowid subquery feeding the `MATCH`; range and low-selectivity predicates run as a SQL `WHERE` after the `MATCH` with the overshoot multiplier applied to K. Callers never see this knob.
...
<projectRoot>/.saivage/rag/
  ...
  <datasetId>/
    ...
    ingest.lock                       // file lock held during ingest
    .corrupted                        // sentinel written when PRAGMA integrity_check fails
```

One `.db` per dataset. Justification: lifecycle ops (`drop`, `rebuild`) become an `unlink` plus a fresh open; corruption in one dataset never blocks queries against the others; per-dataset file locks are local to the file the lock protects; backups are file-granular.
...
```ts
// added to the project config schema in saivage/src/config.ts
rag: z.object({
...
| Embedding-config drift (`provider`/`model`/`dim` differ from `meta`) | `Dataset.open` throws `ConfigDrift` with the expected/actual stamp and the required action: `rebuild`. No partial reuse. |
...
| Concurrent ingest from two processes | `pipeline` acquires an exclusive file lock at `<datasetId>/ingest.lock` using `flock(2)` semantics (advisory, OS-level). Second process throws `IngestLocked` immediately; it does NOT wait. Operator decides whether to retry. |
...
saivage/src/content-store/
  ...
  store/
    ...
    sqlite-records.ts       // single SQLite DB per project holding all records
...
operator / consumer
  ...
  -> collection.upsert(records) writes to records.db (canonical text + metadata)
  -> for each attached index:
       if vector: chunker.chunk(record.text) -> embed -> vec-<collection>.db
       if lexical: FTS5 insert into fts-<collection>.db
```
...
```
<projectRoot>/.saivage/content-store/
  records.db                            // canonical records for all collections
  vec-<collectionId>.db                 // one vector index per collection
  fts-<collectionId>.db                 // one lexical index per collection (when attached)
  ingest.lock                           // single global ingest mutex
```

Single canonical records file simplifies cross-index consistency (one row truly is the source of truth) at the cost of one global ingest mutex and an awkward `drop(collectionId)` that has to delete rows out of a shared table.

#### 3.2.6 Configuration surface

```ts
// added to saivage/src/config.ts
...
The MCP server is opt-in. When enabled, it registers tools like `content_store.query`, `content_store.list_collections`, `content_store.stats`, gated to localhost.

#### 3.2.7 Multi-dataset (collection) isolation

Collections share `records.db` and have one index file each. Trade-off: cross-collection consistency is easier (a single transaction can touch any record), but `drop` is a `DELETE FROM records WHERE collectionId = ?` followed by a `VACUUM` instead of an `unlink`; corruption in `records.db` poisons everything; the global ingest mutex serializes all writers.

#### 3.2.8 Failure modes and design responses

Same list as Proposal A, with these additions:

- `records.db` corruption poisons all collections; recovery requires rebuilding every index. Proposal A scopes corruption to a single dataset.
...
- Cross-collection isolation is weaker (shared records file, one ingest mutex).
...
All entries are ESM-friendly under Node >=24 and ship prebuilt binaries for Linux x64; the implementer must reverify the prebuild matrix before merging.
...
| `proper-lockfile` | Could replace the hand-rolled `flock(2)` lock | Node `fs.constants` and `flock` via `node:fs` suffice; avoid the extra dep |
...
- Confirm the file lock primitive: `node:fs` `flock` advisory lock is the intended choice. If the OS or filesystem (e.g. some network filesystems) does not honor it, the lock degrades to a non-mutex and concurrent ingest could corrupt the file. Document the expectation that `.saivage/rag/` lives on a local filesystem.
...
| Per-dataset isolation | Strong (file per dataset) | Weaker (shared `records.db`) |
| Lifecycle ops cost (`drop`, `rebuild`) | `unlink` + fresh open | `DELETE` + `VACUUM` + rebuild indexes |
| Lines of test surface (rough) | ~12 test files | ~20 test files |
| New npm deps | 6 | 8+ (adds FTS5 helper, MCP transport, potentially LanceDB) |
...
- New dependencies are `better-sqlite3`, `sqlite-vec`, `tree-sitter`, `tree-sitter-typescript`, `tree-sitter-python`, `picomatch`. `engines.node` rises to `>=24.0.0`.
