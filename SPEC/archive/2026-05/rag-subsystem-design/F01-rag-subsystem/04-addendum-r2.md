...
3. A statement that a future "librarian" agent will be the intelligent layer above the RAG library, specified separately. The librarian is one consumer; it is not a gateway, and the library exposes the same stable contract to direct in-process callers and to the librarian alike.
...
The dataset configuration schema in [saivage/src/config.ts](saivage/src/config.ts) gains two related fields: `sources` (the canonical declaration of source roots that watcher and reconcile operate against) and `watch` (the opt-in switch and mode selector for the background watcher).

```ts
type SourceRoot = {
  root: string;               // absolute path
  include?: string[];         // glob array, default ["**/*"]
  exclude?: string[];         // glob array, default []
};
...
type DatasetConfig = {
  // ... existing fields ...
  sources: SourceRoot[];      // canonical declaration of roots; may be empty
  watch: WatchConfig;         // default false
};
```

Semantics of `sources`:

- `sources` is the canonical declaration of which on-disk roots belong to the dataset. The watcher uses `sources` to determine watch scope. `dataset.reconcile()` with no arguments uses `sources` to walk the filesystem and compare against persisted chunk metadata.
- `root` is an absolute path. `include` defaults to `["**/*"]`. `exclude` defaults to `[]`. Per-source `include`/`exclude` compose with the dataset-wide secret exclusion set and the build/cache exclusion set; the combined filter is what both watcher and reconcile honor.
- `sources` MAY be empty. An empty `sources` array means there is nothing for the watcher to watch and nothing for the zero-argument `reconcile()` to walk; both become no-ops. The dataset is still fully usable via explicit `ingest(paths)` for any path the caller chooses.
- The existing per-call `ingest(paths)` operation continues to work and is the right mechanism for ad-hoc ingest of paths NOT covered by `sources` (for example, one-off injection of generated content from elsewhere on disk). Paths supplied only through `ingest(paths)` and not covered by any `sources[*].root` are NOT watched and are NOT visited by zero-argument `reconcile()`; they live as persisted chunks until the caller explicitly deletes or re-ingests them.
...
Watcher scope is the union of `sources[*].root` filtered by each source's `include`/`exclude` and then minus the dataset-wide exclusion sets:
...
Both sets apply to every watcher regardless of dataset source type. The same combined filter is what `dataset.reconcile()` honors when walking `sources[*].root`.

Startup behavior: on dataset startup, if `watch !== false`, the dataset runs `reconcile()` (a `sources`-based mtime + sha256 sweep) BEFORE arming the watcher. If `watch === false`, no startup reconcile runs; reconcile is then an explicitly-callable operation only.
...
The ingest pipeline acquires a per-dataset cross-process lock via `proper-lockfile`. The watcher does NOT bypass it, and it does NOT discard pending work on a timeout:

- When a debounce window closes, the watcher attempts to acquire the same lock.
- If a manual `dataset.ingest()` already holds the lock, the watcher waits indefinitely. There is no timeout that discards the batch.
- While waiting, additional filesystem events MAY accumulate into the same dataset-level "pending watcher batch" marker. There is no separate per-window queue; all events arriving during the wait coalesce into one pending batch.
- When explicit ingest holds the lock, the watcher coalesces all incoming change events into the pending batch and waits. When the lock is released, the watcher acquires it and processes the coalesced batch using the most recent observed state of each affected file (so files that were repeatedly written during the wait are embedded only once at their final content).
- While waiting, the watcher periodically refreshes its own intent so the dataset-level "pending watcher batch" marker remains visible to in-process observers (logs, tests). The refresh has no effect on the `proper-lockfile` lock itself.
- The pending batch is in-memory only. If the process is killed mid-wait, the pending batch is not persisted; on next startup, `reconcile()` will detect any files whose `sha256` has drifted from persisted metadata and route them through ingest, so no observable update is permanently lost.
...
- Walk every `sources[*].root` honoring per-source `include`/`exclude` plus the dataset-wide secret + build/cache exclusions.
...
`reconcile` runs unconditionally on dataset startup when `watch !== false`. It is also exposed as a public, callable operation (see [Section 3.3](#33-public-surface-additions)) so operators and tests can request it explicitly. Zero-argument `dataset.reconcile()` always walks `sources[*].root`; it never visits paths that were only ever supplied through ad-hoc `ingest(paths)` calls and are outside `sources`.
...
| `dataset.reconcile()` | Runs the one-shot mtime + sha256 sweep against persisted chunk metadata across `sources[*].root` and routes the resulting deltas through the ingest pipeline. Used internally on startup; also callable explicitly (e.g. after a `git pull` from an outer script). Acquires the per-dataset lock. |
...
The new failure surface introduced by the watcher is bounded to two named cases:
...
Both are observable via the existing structured-log surface at [saivage/src/log.ts](saivage/src/log.ts). Neither silently corrupts dataset state; the worst case is staleness until the next explicit ingest or `reconcile`. Lock contention is not a failure mode: the watcher waits and coalesces as specified in [Section 3.2.4](#324-lock-awareness).
...
A "librarian" agent is planned as the intelligent layer above the RAG library. Its responsibilities, to be specified in a future, separate agent-design effort and not in this document, are expected to include:
...
- [saivage/src/config.ts](saivage/src/config.ts) — extend the dataset configuration schema with BOTH the `sources` field (canonical declaration of source roots; per-source `include`/`exclude` glob arrays; defaults `include = ["**/*"]`, `exclude = []`; absolute `root` paths) per [Section 3.2.2](#322-configuration-surface) AND the `watch` field per the same section. Default `watch = false`. This is the only edit to [saivage/src/config.ts](saivage/src/config.ts) in this work; no separate batch covers `sources`.
- [saivage/src/rag/dataset.ts](saivage/src/rag/dataset.ts) — add `watch()`, `unwatch()`, and `reconcile()` public operations; instantiate a `WatcherController` lazily when `watch !== false`; wire zero-argument `reconcile()` to walk `sources[*].root`.
...
- With `watch: true` and a non-empty `sources` against a temp directory, a file write triggers re-ingest within debounce + ingest time, and a follow-up `query` returns the new content.
- Build/cache exclusion: writing into `node_modules/` inside a watched `sources[*].root` produces no ingest activity.
- Lock-aware coalescing under indefinite wait: a manual `ingest()` that holds the lock for longer than any prior timeout would have allowed still results in exactly one final state with all watcher-observed changes applied, not in a dropped batch and not in concurrent ingest pipelines.
- Crash-while-pending recovery: a watcher batch pending behind a manual `ingest()` is dropped when the process is killed; on next startup, `reconcile()` over `sources[*].root` detects the changed files via `sha256` drift and routes them through ingest before the watcher re-arms.
...
- `dataset.reconcile()` over `sources`: after the process restarts (simulated by tearing down the manager and re-registering the dataset), files modified while the manager was down are re-ingested before the watcher re-arms.
- Ad-hoc paths outside `sources`: a path supplied only through explicit `ingest(paths)` and not covered by any `sources[*].root` is NOT visited by zero-argument `reconcile()` and is NOT watched.
...
- In [saivage/SPEC/v2/rag/configuration.md](saivage/SPEC/v2/rag/configuration.md): documentation of both the `sources` field shape (roots, per-source `include`/`exclude`, defaults) and the `watch` field shape and defaults.
...
- **Live watcher acceptance:** with a dataset configured `watch: true` and at least one entry in `sources` pointing at a temp directory, a file written into that root is reflected in `query` results within `(debounce window + ingest time)` seconds, with no explicit caller action between the write and the query.

The librarian agent has no acceptance criteria in this work; it is recorded as a future spec only.

## 6. Summary

Three deltas land. The facade is fixed in scope: RAG-only orchestration, with a documented list of features it must refuse. External changes are handled by the existing explicit `ingest`/`delete` API plus an opt-in chokidar watcher with reconcile-on-startup, debounced and lock-aware ingestion, explicit handling of inotify limits and event floods, and an opt-in polling mode for bind-mounts and NFS. The librarian agent is recorded as a future, separately specified consumer; the library does not become it, does not grow convenience methods for it, and does not host an MCP server on its behalf. The implementation sequence absorbs one new batch (B12) between the end-to-end smoke test and the docs batch, and the docs batch picks up the boundary policy and the watcher's operational surface.
