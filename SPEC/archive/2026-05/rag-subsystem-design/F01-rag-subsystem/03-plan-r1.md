...
- Consumer integration with the skills loader, the memory manager at [saivage/src/knowledge/](saivage/src/knowledge/), the planner, or any agent role under [saivage/src/agents/](saivage/src/agents/). No code under `src/agents/`, `src/runtime/`, `src/server/`, `src/knowledge/`, or `web/` is touched.
...
|---|---|---|---|---|---|
| B01 | Node engines + dependencies | none | `package.json` | `T`, `L`, `A` | — |
| B02 | Public types, errors, config slice | `src/rag/types.ts`, `src/rag/errors.ts`, `src/rag/index.ts`, `src/rag/types.test.ts`, `src/rag/errors.test.ts` | `src/config.ts` | `T`, `L`, `U`, `A` | B01 |
| B03 | Vector store seam + sqlite-vec adapter | `src/rag/store/index.ts`, `src/rag/store/sqlite-vec.ts`, `src/rag/store/sql.ts`, `src/rag/store/sqlite-vec.test.ts` | none (gate: `tsconfig.json` only if native interop requires) | `T`, `L`, `U`, `A` | B02 |
| B04 | Embedding provider seam + OpenAI adapter | `src/rag/provider/index.ts`, `src/rag/provider/openai.ts`, `src/rag/provider/openai.test.ts` | none | `T`, `L`, `U`, `A` | B02 |
| B05 | Chunker seam + markdown / code / memory chunkers | `src/rag/chunker/index.ts`, `src/rag/chunker/tokens.ts`, `src/rag/chunker/markdown.ts`, `src/rag/chunker/code.ts`, `src/rag/chunker/memory.ts`, plus `*.test.ts` siblings | none | `T`, `L`, `U`, `A` | B02 |
| B06 | Secret-exclusion guard | `src/rag/security/secrets.ts`, `src/rag/security/secrets.test.ts` | none | `T`, `L`, `U`, `A` | B02 |
| B07 | Ingest pipeline + per-dataset lock + embedding cache | `src/rag/ingest/walker.ts`, `src/rag/ingest/pipeline.ts`, `src/rag/ingest/lock.ts`, `src/rag/cache/embedding-cache.ts`, plus `*.test.ts` siblings | none | `T`, `L`, `U`, `A` | B03, B04, B05, B06 |
| B08 | Query pipeline + drift refusal | `src/rag/query/pipeline.ts`, `src/rag/query/pipeline.test.ts` | none | `T`, `L`, `U`, `A` | B03, B04 |
| B09 | Dataset registry + RagManager lifecycle | `src/rag/manager.ts`, `src/rag/dataset.ts`, `src/rag/registry.ts`, plus `*.test.ts` siblings | none | `T`, `L`, `U`, `A` | B07, B08 |
| B10 | End-to-end ingest+query smoke on fixture corpus | `tests/rag/e2e-ingest-query.test.ts`, `tests/rag/fixtures/docs/**` (~50 markdown docs), `tests/rag/README.md` | none (gate: `eslint.config.js` only if `tests/rag/` needs an override) | `T`, `L`, `U`, `E`, `S`, `A` | B09 |
| B11 | Factual SPEC notes | `SPEC/v2/rag/README.md`, `SPEC/v2/rag/configuration.md`, `SPEC/v2/rag/on-disk-layout.md`, `SPEC/v2/rag/operational-runbook.md` | none | none (docs only) | B09 |
...
If any of the native packages lacks a Node 24 / Linux x64 prebuild, abort B01, downgrade to the most recent line that does, and update the table above. Do NOT introduce source-build fallbacks in this commit.
...
Validation: `T`, `L`, `U` (against `src/rag/types.test.ts` and `src/rag/errors.test.ts`), `A`.
...
Files modified: none. Gate: if `tsconfig.json` requires `"moduleResolution": "nodenext"` or similar for the `sqlite-vec` types, edit [saivage/tsconfig.json](saivage/tsconfig.json) in this batch (single-line change, no broader cleanup).
...
- [saivage/src/rag/dataset.ts](saivage/src/rag/dataset.ts) — `Dataset` implementation that owns a `VectorStore`, an `EmbeddingProvider`, and a `Chunker`, delegates `ingest`/`query` to B07/B08, implements `rebuild` (writes to a sibling `<datasetId>.new/` directory, validates, atomic-renames over `<datasetId>/` on success, deletes any leftover `.new` directory on `open`), implements `drop` (close + `unlink` of `<datasetId>/`), implements `stats` by aggregating `store.stats()` + `meta` rows + the secrets-dropped counter.
...
- [saivage/src/rag/dataset.test.ts](saivage/src/rag/dataset.test.ts), [saivage/src/rag/manager.test.ts](saivage/src/rag/manager.test.ts), [saivage/src/rag/registry.test.ts](saivage/src/rag/registry.test.ts) — `rebuild` produces the same chunk count and same top-K results as a fresh ingest on a fixture; `drop` removes the directory; `register` is idempotent on re-call with identical config and throws `ConfigDriftError` when called with a config that differs in `provider.dim` or `chunker.kind`; registry atomic writes survive simulated mid-write crash.
...
Files modified: none. Gate: if `tests/rag/` requires an eslint override (e.g. to allow `console.log` in the README's documented script shape), add the override to [saivage/eslint.config.js](saivage/eslint.config.js) in this batch as a single localized rule; otherwise do not touch.
...
- [saivage/SPEC/v2/rag/on-disk-layout.md](saivage/SPEC/v2/rag/on-disk-layout.md) — the `<projectRoot>/.saivage/rag/<datasetId>/` layout, the `meta` table contents, the integrity-check + `.corrupted` sentinel + `.ingest.lock` semantics, and the recovery procedure (`rebuild`).
...
Validation: none (docs only; the existing tests must still pass under `A` but no new test coverage is needed).
...
Tests colocated next to source follow the existing Saivage v2 convention visible in [saivage/src/config.test.ts](saivage/src/config.test.ts) and [saivage/src/config-validation.test.ts](saivage/src/config-validation.test.ts); end-to-end tests live under [saivage/tests/rag/](saivage/tests/rag/), which is a new top-level test directory introduced in B10. Existing vitest configuration must pick up both locations; if it does not, B10 widens the include pattern (single-line `vitest.config.ts` edit, scoped to that batch).

## 8. Open questions for the implementer

The implementer MUST resolve the following before the corresponding batch lands; if any answer invalidates an assumption in this plan, update the batch text in-place rather than working around it.

- B01: confirm `sqlite-vec` (latest stable line) ships a Linux x64 prebuilt against Node 24 in the npm package; pinpoint the exact published version and update the table in §6/B01. If no prebuild ships, decide between (a) downgrading the line, (b) accepting a source-build step on `npm install`, or (c) substituting `@lancedb/lancedb` for B03 (the design's documented fallback) — and update §6/B03 accordingly.
...
- B04: identify the exact field in the OpenAI embeddings response (or the OpenAI account-level metadata) that the implementer will record as `releaseFingerprint`. If no such field is published, document the derivation rule (e.g. `${model}-${YYYYQn}`) in B11 and stamp accordingly.
...
- B09: decide whether `Dataset.rebuild` writes to `<datasetId>.new/` (preferred — survives crashes) or to `<datasetId>/store.db.new` (smaller surface, but harder to atomically swap the full directory). Default to the directory-level rename documented in §6/B09; record any deviation here.
- B10: confirm the test runner picks up `tests/rag/` without configuration changes; if vitest's `include` default does not cover it, update `vitest.config.ts` in B10 and add an entry to the batch row.
