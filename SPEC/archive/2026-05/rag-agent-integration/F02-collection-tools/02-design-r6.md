...
const manager = await createRagManager({
  ...
  datasets: ragDatasets,                              // SAME array passed below
  providerOptions,
});
const ragService: RagService = {
  manager,
  datasets: ragDatasets,                              // shared array (manager + service)
  watchStatus: new Map(),
  adminRoles: new Set(),
  control: { busy: false },
  enabled: config.rag.enabled,
...
[src/rag/manager.ts](src/rag/manager.ts#L34-L40). `projectId` is sourced from `project.config.project_name`, where
`project` is the project store in the bootstrap scope and
`project_name` is the only project-id-shaped field on the
existing `ProjectConfig` schema (see
[src/store/project.ts](src/store/project.ts#L126)). The same `ragDatasets` array object is passed to
the manager and stored on `RagService.datasets`; this is required
because `RagManager` closes over the array reference for
lookups (see [src/rag/manager.ts](src/rag/manager.ts#L91-L113)),
and all `rag_register` / `rag_drop` runtime mutations must be
visible to subsequent `get`/`ingest`/`query`/`stats`/`drop`
calls.
