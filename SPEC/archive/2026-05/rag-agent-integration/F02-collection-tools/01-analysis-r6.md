...
1. `rag_register` resolves `sources[0].root` to realpath; rejects
   `RAG_BLOCKED_PATH` if `path.relative(realpath(ctx.projectRoot),
   realpath(sources[0].root))` is empty or starts with `..` or is
   absolute. Stores the resolved realpath on the persisted config.
2. The hardened internal walker enforces same containment against
   the stored dataset root for every walked entry; any escape causes
   the ingest run to abort with `RAG_BLOCKED_PATH` (propagated as a
   typed error class).
...
4. Run `manager.ingest(newConfig.id, { kind: "fs", root:
   ...
   symlink escapes; any escape that reaches the handler surfaces as
   `RAG_BLOCKED_PATH`.
...
| `RAG_BLOCKED_PATH`            | `shouldSkipPath` rejection; realpath containment failure at register; internal walker escape on ingest. |
