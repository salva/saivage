...
- Identify and patch the operator-driven call site (CLI entry that builds the bootstrap-time `ToolCallContext` in `src/cli.ts` and `src/server/server.ts` for operator HTTP/Slash flows) to set `operatorContext: true` only there.
...
```bash
npm run typecheck \
  && npx eslint src/mcp/toolContext.ts src/runtime/dispatcher.ts src/agents/chat.ts src/cli.ts src/server/server.ts \
