...
`checkScope` is the existing function from analysis §5 with
signature `(role, op, kind, scope, scope_ref, ctx)`. The
Librarian update path uses op `"update_memory"` against the
