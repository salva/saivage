...
Every step is a single commit and is reverted in isolation with `git revert <step-hash> && npm install` (the `npm install` is unnecessary for the doc-only steps (f) and (g)). The git revert of step (a) restores both `package.json` and `package-lock.json` together because they are part of the same commit, so step (a) is itself recoverable to its parent state with one command.

LIFO rollback of a contiguous range is the normal procedure for any multi-step revert. If steps (b) through (e) have already landed and step (a) must be reverted, the implementer reverts (e), (d), (c), (b), then (a) — each as its own `git revert` commit, in last-in-first-out order. This rule is uniform across the topic and is consistent with the per-batch commit structure; it is not an exception carved out for step (a). Standalone rollback of any single step (including (a)) requires that no later step is currently in place; if later steps have landed, revert them first in LIFO order.
...
Pass criterion: all three commands exit `0`. Rollback: `git revert HEAD && rm -rf node_modules && npm install`. If steps (b)–(e) have already landed when step (a) must be reverted, revert them first in LIFO order (see §1.3).
...
- [F01 plan, revision r2](../rag-subsystem-design/F01-rag-subsystem/03-plan-r2.md) — drop the engines-pin step from B01 and add a "Prerequisite: F02 step (a) merged" line.
