...
Binding design: [saivage/SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/02-design-r3.md](saivage/SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/02-design-r3.md).
...
Every batch ends with at least `T+L+A`. Batches (c) and (f) additionally run `F`. Every runtime batch enforces `W`.
...
| F | `PATH=~/.local/node-24/bin:$PATH npm audit --json` | Final-gate: zero `high`, zero `critical`, and zero entries in the `vulnerabilities` map keyed by `ws`, `qs`, `protobufjs`, `happy-dom`. Residual moderates allowed only in the `esbuild → vite → vitepress → vitepress-plugin-mermaid` chain. |
...
| (c) | `npm audit fix` (no `--force`) — closes `ws`, `qs`, `protobufjs` | [saivage/package-lock.json](saivage/package-lock.json); [saivage/package.json](saivage/package.json) only if the `overrides` fallback fires | T+L+A+F+W | (b) | `git revert <hash> && npm install` |
...
| (f) | Final `npm audit` evidence capture | This plan file only — [saivage/SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/03-plan-r1.md](saivage/SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/03-plan-r1.md) §9 evidence block | T+L+A+F+W | (e) | `git revert <hash>` |
...
PATH=~/.local/node-24/bin:$PATH npm run lint
PATH=~/.local/node-24/bin:$PATH npm test
PATH=~/.local/node-24/bin:$PATH npm audit --json > /home/salva/g/ml/tmp/f02-c-audit.json
node -e '
  const a = require("/home/salva/g/ml/tmp/f02-c-audit.json");
...
Pass criteria: T+L+A+W green; the `vulnerabilities` map of `npm audit --json` has no key named `ws`, `qs`, or `protobufjs`, irrespective of advisory id. There is no "same severity under a different advisory" escape hatch — those three names must be absent.
...
- [saivage/SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/03-plan-r1.md](saivage/SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/03-plan-r1.md) — §9 evidence block only.
...
git add SPEC/2026-05/node24-deps-refresh/F02-node24-deps-refresh/03-plan-r1.md
git commit -m "F02(f): record final audit transcript and residual advisories"
```

Validation (the final audit contract; see §7):

```bash
PATH=~/.local/node-24/bin:$PATH npm run typecheck
PATH=~/.local/node-24/bin:$PATH npm run lint
PATH=~/.local/node-24/bin:$PATH npm test
PATH=~/.local/node-24/bin:$PATH npm audit --json > /home/salva/g/ml/tmp/f02-f-audit.json
node -e '
  const a = require("/home/salva/g/ml/tmp/f02-f-audit.json");
  const v = a.vulnerabilities || {};
...
Pass criteria: T+L+A+W green; the node check above exits `0`. If the check fails, F02 returns to batch (c) or earlier — F02 does not chase post-(c) advisories.
...
git diff --name-only HEAD~1 HEAD \
    | grep -vE '^SPEC/2026-05/rag-subsystem-design/F01-rag-subsystem(\.md|/03-plan-r2\.md)$' \
    | { ! grep . ; }
# Negative grep: no leftover engines-pin claim in F01.
grep -nE 'engines\.node|>=24\.0\.0' \
    SPEC/2026-05/rag-subsystem-design/F01-rag-subsystem.md \
    SPEC/2026-05/rag-subsystem-design/F01-rag-subsystem/03-plan-r2.md \
    && { echo "FAIL: residual engines-pin reference in F01"; exit 1; } \
    || echo "ok: no engines-pin reference remains in F01"
```

Pass criteria: the path-set guards exit `0` (no path outside the two F01 files changed); the negative grep finds zero hits. There is no T/L/A/F runtime gate — this is a documentation-only commit.
...
> Saivage v2 runs on the latest stable Node.js (>= 24, current LTS at design time) with ESM and TypeScript. The Node engine pin at `>=24.0.0` in [saivage/package.json](saivage/package.json) is owned by [F02 — Node 24 migration and dependency refresh](../node24-deps-refresh/F02-node24-deps-refresh.md) step (a); F01 assumes that step has merged and does not modify the `engines.node` field. F01 analysis and design must assume Node 24+ APIs (including `node:sqlite` if it helps, `worker_threads` improvements, native `fetch`, etc.). Do NOT design around Node 20 limitations. Dependencies must be ESM-friendly and must have working prebuilt binaries for current Node on Linux x64.
...
> The implementation lands new RAG-specific dependencies (`better-sqlite3`, `sqlite-vec`, `tree-sitter`, `tree-sitter-typescript`, `tree-sitter-python`, `picomatch`, `proper-lockfile`) in [saivage/package.json](saivage/package.json). The Node engine pin at `>=24.0.0` is a prerequisite owned by [F02](../../node24-deps-refresh/F02-node24-deps-refresh.md) step (a); F01 does not modify `engines.node`.
...
>
> Prerequisite: [F02](../../node24-deps-refresh/F02-node24-deps-refresh.md) step (a) merged — `engines.node` is already `>=24.0.0` on master.
>
...
After both edits land, the negative grep in §6.g validation must find zero hits for `engines\.node` and `>=24\.0\.0` in the two amended files. If the grep finds anything, the amendment is incomplete and a follow-up commit within (g)'s scope is required.

---

## 9. Evidence section (populated by batch (f))

This section is empty until batch (f) lands. The commit for (f) pastes the verbatim contents of `/home/salva/g/ml/tmp/f02-evidence.txt` between the markers below.
...
- [ ] (c) merged; T+L+A+F+W green; `ws`, `qs`, `protobufjs` absent from `npm audit --json` `vulnerabilities` map.
...
- [ ] (f) merged; §7 final audit contract holds; §9 evidence populated.
- [ ] (g) merged; path-set guards green; negative grep finds zero engines-pin references in F01.
